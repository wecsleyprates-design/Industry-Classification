...
def _sql() -> None:
    st.markdown(panel("SQL Runner", "fa-terminal", object_key="explorer.sql"), unsafe_allow_html=True)
...
def _health() -> None:
    st.markdown(panel("Dataset Health", "fa-heart-pulse", object_key="dataset.health"), unsafe_allow_html=True)
...
def _joins() -> None:
    st.markdown(panel("Join & Key Validation", "fa-arrows-left-right-to-line", object_key="joins.validation"), unsafe_allow_html=True)
