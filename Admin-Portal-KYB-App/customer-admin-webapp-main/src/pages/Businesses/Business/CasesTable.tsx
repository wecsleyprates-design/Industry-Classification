	...
	const columns: column[] = [
		{
			...
			content: (item) => (
				<Link
					to={`${getSlugReplacedURL(
						URL.BUSINESS_APPLICANT_CASE_DETAILS,
						[businessId, item?.id],
						[":slug", ":caseId"],
					)}?tab=2`}
	...
	return (
		<React.Fragment>
			<div className="flex justify-end w-full min-w-full px-4 py-2 border-b max-w-7xl sm:px-2 lg:px-4">
				<div className="flex flex-col justify-between w-full gap-2 md:flex-grow sm:flex-row lg:gap-4">
					<div className="w-52">
						{tableTitle ? (
							<span className="w-52 text-[#111827] text-xs mt-2 font-medium">
								{tableTitle}
							</span>
						) : null}
					</div>
					<div className="flex flex-col justify-end w-full gap-2 md:flex-grow sm:flex-row lg:gap-4 ">
						<Filter
						...
						/>
						<SearchBox
							placeholder="Search by ticket number, applicant name and business name"
							value={
								payload.search
