...
import { useResendInviteQuery } from "@/services/queries/auth.query";
import { useGetWorthScoreDate } from "@/services/queries/businesses.query";
import {
	useArchiveCaseQuery,
	useGetCaseByIdQuery,
...
import CaseTabValuesResults from "./CaseTabValuesResults";
import ViewTabLayout, { BANKING_TAB_ID } from "./TabViewLayout";

import { CASE_STATUS_ENUM } from "@/constants/CaseStatus";
import type { CaseTabValuesRowId } from "@/constants/caseTabValues";
import { CASE_TAB_VALUES_NAVIGATION_MAP } from "@/constants/caseTabValues";
			...
			<div>
				<div className="grid grid-cols-1 my-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
					...
					<div>
						...
						<p className="py-2 text-sm font-medium tracking-tight text-slate-800">
							{convertToLocalDate(
								caseData?.data?.status_history[0]?.created_at,
								"MM-DD-YYYY - h:mmA",
							)}
						</p>
					</div>
					<div className="z-50 mx-0 overflow-visible sm:-mx-2">
						<p className="py-2 text-xs font-normal tracking-tight text-gray-500">
							Status
						</p>
						<p className="flex py-2 text-sm font-medium tracking-tight truncate text-slate-800">
					...
					<div className="grid items-center grid-cols-2 gap-2 mb-2 sm:flex sm:flex-col sm:items-start">
						<p className="text-[10px] text-[#5E5E5E] font-normal tracking-tight py-2">
							Risk alerts
						</p>
...
const CaseDetails = () => {
	...
	const { data: caseTabValuesData, isLoading: caseTabValuesLoading } =
		useCaseTabValuesQuery({
			businessId: getApplicantData?.data?.business_id ?? null,
			caseId: slug ?? null,
		});
	...
	useEffect(() => {
		if (getApplicantError && isAxiosError(getApplicantError)) {
			...
			if (getApplicantError?.response?.status === STATUS_CODES.NOT_FOUND) {
				// navigate to not found screen
				navigate(URL.NOT_FOUND, { replace: true });
			} else if (
				getApplicantError?.response?.status !== STATUS_CODES.UNAUTHORIZED
			) {
				navigate(URL.CASE);
			}
		}
	}, [JSON.stringify(getApplicantError)]);
	...
	useEffect(() => {
		const tempButtons = [
			<Button
				...
				onClick={() => {
					navigate(`${URL.CASE}?tableName=${tableName}`, { replace: true });
				}}
				color="transparent"
			>
			...
			(applicantData?.status?.id === "INVITED" ||
				applicantData?.status?.id === "INVITE_EXPIRED") && (
				<Button
					key={"invite"}
				...
				>
					<span className="text-sm font-medium text-white">Resend invite</span>
				</Button>
			),
		...
		if (
			...
			tempButtons.push(
				<div className="relative inline-block rounded-lg h-11 w-30">
					<Button
					...
					>
						<div className="max-w-[18px]">
							<PencilSquareIcon className="h-[18px] text-black min-w-[18px]" />
						</div>
						Edit Application
					</Button>
					<div
		...
		if (
			...
			switch (businessReportStatusData?.data.status) {
				...
				case "REGENERATED_SUCCESSFULLY":
					tempButtons.push(
						...
						>
							...
							>
								<DocumentCheckIcon className="min-w-4 max-w-4 h-4 text-[#1F2937]" />
								View Report
							</Button>
				...
				case "REGENERATION_IN_PROGRESS":
					tempButtons.push(
						<Button
							className="px-2 rounded-lg flex content-center h-11 items-center justify-center gap-2 cursor-not-allowed text-[#6B7280]"
							color="grey"
							outline
						>
				...
				case "IN_PROGRESS":
					tempButtons.push(
						<Button
							...
							onClick={() => {}}
						>
							<ClockIcon className="min-w-4 max-w-4 h-4 text-[#6B7280]" />
							Report Processing
						</Button>,
					);
					break;
				case "FAILED":
					tempButtons.push(
						...
						>
							<Button
								className="px-2 rounded-lg flex content-center h-11 items-center justify-center gap-2 text-[#6B7280] opacity-90"
								color="grey"
								outline
								disabled
								onClick={() => {
							...
							>
								<ArrowDownTrayIcon className="min-w-4 max-w-4 h-4 text-[#6B7280]" />
								Report Unavailable
							</Button>
							{!isRegenerating && (
								<Button
				...
				case "REQUESTED":
					tempButtons.push(
						<Button
							className="px-2 rounded-lg flex content-center h-11 items-center justify-center gap-2 text-[#6B7280]"
							color="grey"
							outline
							onClick={() => {}}
						>
				...
				case "DOWNLOAD_REPORT_AVAILABLE":
					tempButtons.push(
						...
						>
							<ArrowDownTrayIcon className="h-4 min-w-4 max-w-4" />
							Download Report
						</Button>,
					);
					break;
				case "DOWNLOAD_REPORT_UNAVAILABLE":
					tempButtons.push(
						<Button
							className="px-2 rounded-lg flex content-center h-11 items-center justify-center gap-2 cursor-not-allowed text-[#6B7280] opacity-90"
							color="grey"
							outline
							disabled
						>
	...
	return (
		<React.Fragment>
			{(archiveCaseLoading || isLoading) && <FullPageLoader />}
			{isArchiveModalOpen && (
				<WarningModal
			...
			<Card
				headerComponent={
					<PageTitle
						...
						backLocation={`${URL.CASE}?tableName=${tableName}`}
						isBackAllowed={false}
						businessName={applicantData?.business?.name}
					/>
				}
					...
					{applicantData?.business_id && applicantData?.id && (
						<div className="flex flex-row w-full">
							<div className="w-full p-6 pt-2 mt-8 bg-white border rounded-lg shadow">
								<CaseTabValuesResults
									data={caseTabValuesData}
									isLoading={caseTabValuesLoading}
									onRowClick={(rowId: CaseTabValuesRowId) => {
										const target = CASE_TAB_VALUES_NAVIGATION_MAP[rowId];
										if (target?.mainTab === "banking") {
											setActiveTabId(BANKING_TAB_ID);
			...
			>
				...
				<form className="px-4 py-5 space-y-6 sm:p-6" action="#" method="POST">
					<div className="mt-6 text-2xl leading-9 tracking-tight text-gray-900">
						<p className="font-bold text-[24px] sm:text-[24px] text-[#333]">
							Add Comments
						</p>
						<p className="mt-1 text-[15px] font-normal text-gray-400 tracking-wide"></p>
					</div>
					...
					<TextArea
						...
						defaultValue={""}
						icons={[<AttachmentIcon key={"attachmentIcon"} />]}
						footerText="0/500 characters"
					/>

					<div className="text-center">
						<Button
							className="mt-3 h-14 bg-black w-full font-bold p-1.5 text-white shadow-sm focus-visible:outline"
							type="submit"
							isLoading={loading}
						>
