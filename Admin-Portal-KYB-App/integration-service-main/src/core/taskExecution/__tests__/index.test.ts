...
describe("core/taskExecution/index", () => {
	...
	describe("executeIntegrationTask", () => {
		...
		it("should return early when task_status is SKIPPED", async () => {
			const task = createMockTask({ task_status: TASK_STATUS.SKIPPED });
			await executeIntegrationTask(task);
			expect(mockGetConnectionById).not.toHaveBeenCalled();
		});
			...
			expect(mockSqlTransaction).toHaveBeenCalledWith(
				...
				expect.arrayContaining([
					...
					expect.arrayContaining([
						...
						task.id,
						TASK_STATUS.FAILED,
						expect.stringContaining("No connection found for task")
					])
				])
			);
		});
	...
	describe("noOperation (via TASK_CODE_TO_EXECUTION_FUNCTION.fetch_commerce_payments)", () => {
		it("should call prepareIntegrationDataForScore without updating task status", async () => {
			const task = createMockTask({ task_code: "fetch_commerce_payments" });
			const connection = createMockConnection();

			await TASK_CODE_TO_EXECUTION_FUNCTION.fetch_commerce_payments(connection, task);
