...
> = {
	...
	fetch_watchlist_hits: deferTask
};
...
async function deferTask(_connection, task): Promise<void> {
	logger.debug(
		{ task },
		`deferTask: businessId=${task.business_id}, taskCode=${task.task_code} -- should be handled outside this context`
	);
}
...
export async function executeIntegrationTask<T = any>(
	...
	if (
		[TASK_STATUS.IN_PROGRESS, TASK_STATUS.STARTED, TASK_STATUS.SUCCESS, TASK_STATUS.SKIPPED].includes(
			task.task_status as "STARTED" | "IN_PROGRESS" | "SUCCESS" | "SKIPPED"
		)
	) {
		return;
	}
