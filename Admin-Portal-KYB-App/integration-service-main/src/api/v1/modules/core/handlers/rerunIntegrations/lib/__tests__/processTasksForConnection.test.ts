...
describe("processTasksForConnection", () => {
	...
	const scoreTriggerId = "test-score-trigger-id" as UUID;
	...
	describe("when connection cannot be retrieved", () => {
		it("should return error when getOrCreateConnection returns null", async () => {
			...
			/** Act */
			const result = await processTasksForConnection(businessID, scoreTriggerId, conn);
	...
	describe("when no task codes are available", () => {
		...
		it("should return error with requested codes when no matching task codes found", async () => {
			...
			/** Act */
			const result = await processTasksForConnection(businessID, scoreTriggerId, conn, ["task2"]);
	...
	describe("when processing tasks", () => {
		it("should process tasks successfully", async () => {
			...
			const scoreTriggerId = "00000000-0000-0000-0000-000000000001" as UUID;
		...
		it("should pass scoreTriggerId to adapter.process when provided", async () => {
			...
			/** Act */
			await processTasksForConnection(businessID, scoreTriggerId, conn, undefined, undefined);

			/** Assert */
			expect(mockAdapter.process).toHaveBeenCalledWith(expect.objectContaining({ scoreTriggerId }));
		});
		...
		it("should pass metadata to adapter process", async () => {
			...
			/** Act */
			await processTasksForConnection(businessID, scoreTriggerId, conn, undefined, metadata);
		...
		it("should pass requestedTaskCodes to getTaskCodesToRun", async () => {
			...
			/** Act */
			await processTasksForConnection(businessID, scoreTriggerId, conn, ["task1", "task2"]);
