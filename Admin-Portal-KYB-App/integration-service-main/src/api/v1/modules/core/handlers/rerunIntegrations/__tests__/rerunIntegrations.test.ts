...
describe("rerunIntegrations", () => {
	...
	describe("when processing succeeds", () => {
		it("should process tasks and return created tasks", async () => {
			...
			expect(mockProcessTasksForConnection).toHaveBeenCalledWith(businessID, MOCK_SCORE_TRIGGER_ID, conn, [], metadata);
		});
		...
		it("should pass task_codes filter to processTasksForConnection", async () => {
			...
			expect(mockProcessTasksForConnection).toHaveBeenCalledWith(
				businessID,
				MOCK_SCORE_TRIGGER_ID,
				conn,
				taskCodes,
				metadata
			);
	...
	describe("completion tracking", () => {
		...
		it("should throw an error when manager.initialize fails", async () => {
			const conn = createMockConnection();
			mockGetConnectionsAndTasks.mockResolvedValue([conn]);
			mockProcessTasksForConnection.mockResolvedValue({ tasksCreated: [], errors: [] });
			mockInitialize.mockRejectedValue(new Error("DB error"));

			await expect(rerunIntegrations({ businessID }, { platform_codes: ["MOCK"] })).rejects.toThrow(
				"Failed to create score trigger and initialize tracking"
			);
		});
	});
