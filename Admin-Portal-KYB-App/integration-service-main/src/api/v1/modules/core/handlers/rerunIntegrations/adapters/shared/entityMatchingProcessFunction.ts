...
> = async params => {
	...
	const entityMatchingTaskId = await entityMatchingPlatform.getOrCreateTaskForCode({
		...
		scoreTriggerId,
		/**
		 * Unlike the defaultAdapterProcessFunction, which uses metadata as the condition,
		 * we instead check if there's an existing task with the same score trigger id.
		 * This is because entity matching task metadata is overwritten once it's sent to the warehouse service.
		 * So, to prevent multiple adapters from creating multiple entity matching tasks for the same score trigger,
		 * we check if there's an existing task with the same score trigger id.
		 *
		 * If there's no existing task with the same score trigger id, a new task will be created.
		 */
		conditions: [{ column: "business_score_trigger_id", operator: "=", value: scoreTriggerId }]
	});

	if (!entityMatchingTaskId) {
		const errorMessage = `Could not get or create EntityMatching task for ${platform_code}`;
