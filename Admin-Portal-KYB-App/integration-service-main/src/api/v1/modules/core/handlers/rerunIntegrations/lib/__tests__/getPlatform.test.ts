...
import type { Equifax } from "#lib/equifax/equifax";
...
describe("getPlatform", () => {
	...
	describe("EQUIFAX platform", () => {
		it("should initialize Equifax via strategy factory and return instance", async () => {
			/** Arrange */
			const dbConnection = createMockConnection({ platform_id: INTEGRATION_ID.EQUIFAX });
			const mockEquifax = createMockPlatform() as unknown as Equifax;
			mockStrategyPlatformFactory.mockResolvedValue(mockEquifax);

			/** Act */
			const result = await getPlatform<Equifax>(dbConnection);
			...
			expect(result).toBe(mockEquifax);
		});

		it.each([
			["null", null],
			["undefined", undefined]
		] as const)("should throw when strategyPlatformFactory returns %s", async (_, value) => {
			/** Arrange */
			const dbConnection = createMockConnection({ platform_id: INTEGRATION_ID.EQUIFAX });
			mockStrategyPlatformFactory.mockResolvedValue(value);

			/** Act & Assert */
			await expect(getPlatform<Equifax>(dbConnection)).rejects.toThrow("Failed to initialize Equifax platform");
		});
		...
		it("should propagate error from strategyPlatformFactory", async () => {
			...
			/** Act & Assert */
			await expect(getPlatform<Equifax>(dbConnection)).rejects.toThrow("Strategy factory failed");
		});
	});
