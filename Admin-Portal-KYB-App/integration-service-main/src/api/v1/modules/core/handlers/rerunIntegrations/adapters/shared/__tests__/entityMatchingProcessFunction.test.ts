...
describe("entityMatchingProcessFunction", () => {
	...
	const createParams = (
		...
		scoreTriggerId: "00000000-0000-0000-0000-000000000001" as UUID,
	...
	describe("when EntityMatching is enabled", () => {
		...
		it("should pass correct conditions to EntityMatching task", async () => {
			/** Arrange */
			const scoreTriggerId = "00000000-0000-0000-0000-000000000002" as UUID;
			...
			const params = createParams({ platform: mockIntegrationPlatform, metadata, scoreTriggerId });
			...
			expect(mockEMPlatform.getOrCreateTaskForCode).toHaveBeenCalledWith({
				...
				conditions: [
					{
						column: "business_score_trigger_id",
						operator: "=",
						value: scoreTriggerId
					}
				]
			});
		});
	...
	describe("EntityMatching task creation failures", () => {
		...
		it("should throw error and log warning when task creation fails", async () => {
			...
			/** Act & Assert */
			await expect(entityMatchingProcessFunction(params)).rejects.toThrow(
				`Could not get or create EntityMatching task for test_platform`
			);
			expect(mockLoggerWarn).toHaveBeenCalledWith("Could not get or create EntityMatching task for test_platform");
		});
