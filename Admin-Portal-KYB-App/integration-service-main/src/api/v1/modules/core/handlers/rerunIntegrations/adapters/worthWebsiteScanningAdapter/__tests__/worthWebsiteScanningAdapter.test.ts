...
import { getWorthWebsiteScanningService, WorthWebsiteScanning } from "#lib/worthWebsiteScanning/worthWebsiteScanning";
...
describe("worthWebsiteScanningAdapter", () => {
	...
	describe("process", () => {
		...
		it("should not call fetchWebsiteDetails if task creation fails", async () => {
			...
			await expect(worthWebsiteScanningAdapter.process(createProcessParams())).rejects.toThrow("Task creation failed");
