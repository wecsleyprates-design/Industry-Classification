...
import { normalizeQueryAcquirerId } from "../controller";
...
-----END PRIVATE KEY-----`;
	...
	describe("normalizeQueryAcquirerId", () => {
		it("should return a trimmed string for a valid string value", () => {
			expect(normalizeQueryAcquirerId("  12345  ")).toBe("12345");
		});

		it("should return undefined for an empty or whitespace-only string", () => {
			expect(normalizeQueryAcquirerId("")).toBeUndefined();
			expect(normalizeQueryAcquirerId("   ")).toBeUndefined();
		});

		it("should return undefined when value is undefined", () => {
			expect(normalizeQueryAcquirerId(undefined)).toBeUndefined();
		});

		it("should throw 400 when value is an array", () => {
			expect(() => normalizeQueryAcquirerId(["12345", "67890"])).toThrow("Invalid acquirerId: must be a single string value");
		});

		it("should throw 400 when value is a ParsedQs object", () => {
			expect(() => normalizeQueryAcquirerId({ nested: "value" })).toThrow("Invalid acquirerId: must be a single string value");
		});
	});
	...
	describe("MatchConnection Multi-ICA Logic", () => {
		...
		describe("getDefaultICA (via checkConnection mock payload)", () => {
			...
			it("should use the acquirerId override instead of stored ICAs when provided", async () => {
				const secrets = {
					storage_data: JSON.stringify({
						customerName: "Test",
						consumerKey: "key",
				...
				await matchConnection.checkConnection("customer-id", "999");
