...
export const normalizeQueryAcquirerId = (rawAcquirerId: string | string[] | object | undefined): string | undefined => {
	if (Array.isArray(rawAcquirerId) || (typeof rawAcquirerId === "object" && rawAcquirerId !== null)) {
		throw new VerificationApiError(
			"Invalid acquirerId: must be a single string value",
			StatusCodes.BAD_REQUEST,
			ERROR_CODES.INVALID
		);
	}
	if (typeof rawAcquirerId === "string") {
		const trimmed = rawAcquirerId.trim();
...
export const controller = {
	checkConnectionStatus: catchAsync(async (req: Request, res: Response) => {
		...
		const acquirerId = normalizeQueryAcquirerId(req.query.acquirerId);
		try {
			const statusConnection = await matchConnection.checkConnection(customerId, acquirerId);
