...
class MatchConnection {
	 ...
	 * @param acquirerId - Optional ICA to use for the test instead of the one stored in AWS
	 * @returns Promise<connectionResult> - Connection status with details
	 */
	public async checkConnection(customerId: string, acquirerId?: string): Promise<connectionResult> {
		try {
			// Get customer credentials
			let customerCredentials: Secrets | null = null;
			try {
				customerCredentials = await this.getCustomerCredentials(customerId);
				logger.info({ customerId }, `checkConnection: Retrieved credentials for customer`);
			} catch (error) {
			...
			if (!customerCredentials) {
				logger.warn({ customerId }, `checkConnection: No credentials found for customer`);
				return {
			...
			if (!expiryDate) {
				logger.warn(
					{ customerId },
					"checkConnection: No certificate expiry metadata found — credentials may have been saved before expiry tracking was added"
				);
			}
			...
			const method = "post";
			const normalizedAcquirerId = acquirerId?.trim();
			const icas = normalizedAcquirerId
				? parseICAs(normalizedAcquirerId)
			...
			try {
				...
				logger.info(
					...
					"checkConnection: response received"
				);
			} catch (error) {
