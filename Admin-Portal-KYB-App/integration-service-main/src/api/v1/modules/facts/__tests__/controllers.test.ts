...
describe("controller.getBusinessKybDetails — watchlist facts", () => {
	...
	it("should strip adverse_media_hits from KYB response", async () => {
		mockGetResults.mockResolvedValue({
			watchlist: { value: { metadata: [], message: "No hits" } },
			watchlist_hits: { value: 0 },
			adverse_media_hits: { value: 5 },
			business_name: { value: "Test Corp" }
		});
		...
		const data = cacheOutput.cacheOutput?.data as Record<string, any>;
		expect(data.adverse_media_hits).toBeUndefined();
		expect(data.business_name.value).toBe("Test Corp");
		expect(data.watchlist_hits.value).toBe(0);
	});

	it("should strip adverse_media_hits even when it contains raw hit array", async () => {
		mockGetResults.mockResolvedValue({
			watchlist: { value: { metadata: [], message: "No hits" } },
			watchlist_hits: { value: 0 },
			adverse_media_hits: {
				value: [
					{ listType: "ADVERSE_MEDIA", listName: "Negative News", confidence: 0.8 },
					{ listType: "ADVERSE_MEDIA", listName: "Media Report", confidence: 0.6 }
				]
			}
		});
	...
	it("should not throw when getResults returns null or undefined", async () => {
		mockGetResults.mockResolvedValue(null);

		const { req, res, next } = makeMockReqResNext();
		await expect(controller.getBusinessKybDetails(req, res, next)).resolves.not.toThrow();
	});
});
