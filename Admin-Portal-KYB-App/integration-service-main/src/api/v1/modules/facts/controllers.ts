...
export const controller = {
	...
	getBusinessKybDetails: catchAsync(async (req, res: Response, next: NextFunction) => {
		...
		// Adverse media is served exclusively by the /adverse-media endpoint;
		// remove it from the KYB response to avoid duplication.
		if (data && typeof data === "object") {
			delete (data as Record<string, unknown>).adverse_media_hits;
		}
