...
import { processBaselayerWebhookAsync } from "#lib/baselayer/baselayerBusiness";
import {
...
export const controller = {
	...
	handleBaselayerWebhook: catchAsync(async (req, res) => {
		const body = req.body as Record<string, unknown>;
		// Process before responding (same pattern as handleVerificationWebhook) so failures return non-2xx
		// and Svix/Baselayer can retry; fire-and-forget would ack before persistence and risk data loss on crash.
		await processBaselayerWebhookAsync(body);
		res.status(200).jsend.success("Baselayer webhook processed successfully.");
	}),
