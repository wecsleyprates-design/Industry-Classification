...
const serpApiBaseUrl = envConfig.SERP_API_BASE_URL.replace(/\/+$/, "");
...
export class DataScrapeService extends TaskManager {
	...
	async scrapeGoogleReviews(googleReviewsLink: string): Promise<GoogleReview[]> {
		...
		try {
			const modifiedUrl = googleReviewsLink.replace(
				`${serpApiBaseUrl}/search.json?`,
				`${serpApiBaseUrl}/search.json?api_key=${serpApiKey}&`
			);
	...
	async searchSerpAPI({
		...
		try {
			...
			const serpMapsQuery = `api_key=${serpApiKey}&engine=google_maps&type=search&google_domain=google.com&q=${q}&hl=en&ll=${ll}`;
			const url = `${serpApiBaseUrl}/search?${serpMapsQuery}`;
