...
describe("AdverseMedia - Region Filter Logic", () => {
	...
	describe("fetchAdverseMediaData - multi-source (SerpAPI + Trulioo)", () => {
		...
		it("should include deterministic tiebreaker (ORDER BY id) in query", async () => {
			tracker.on.select(/"adverse_media" where/).response([
				{ adverse_media_id: "am-1" }
			]);
			tracker.on.select(/adverse_media_articles/).response([
				{ id: "a1", risk_level: "HIGH", entity_focus_score: 10, final_score: 10 },
				{ id: "a2", risk_level: "HIGH", entity_focus_score: 10, final_score: 10 }
			]);

			await adverseMedia.getAdverseMediaByBusinessId(
				{ businessId: "biz-order" },
				{ sortFields: [{ field: "entity_focus_score", order: "desc" }] }
			);

			const articlesQuery = tracker.history.select.find(
				(q: any) => q.sql?.includes("adverse_media_articles")
			);
			expect(articlesQuery).toBeDefined();
			const sql = articlesQuery!.sql;
			expect(sql).toContain("order by");
			expect(sql).toMatch(/order by.*"entity_focus_score".*"id"/is);
		});
	});
