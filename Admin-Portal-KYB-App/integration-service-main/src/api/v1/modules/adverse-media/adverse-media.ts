		...
		try {
			...
			const articles = await db("integration_data.adverse_media_articles")
				...
				.modify(queryBuilder => {
					...
					} else if (sortField.field === "date") {
						const nullsPos = sortField.order === "desc" ? "NULLS LAST" : "NULLS FIRST";
						queryBuilder.orderByRaw(`date ${sortField.order === "desc" ? "DESC" : "ASC"} ${nullsPos}`);
					} else {
