...
import { recordTimeToCriticalDataMetrics } from "#metrics";
...
export class TaskManager extends AbstractTaskManager {
	...
	public async updateTaskStatus<T = any>(
		...
		if (task_status === TASK_STATUS.SUCCESS) {
			recordTimeToCriticalDataMetrics(enrichedTask);
		}
	...
	public async getTasksForCode({ taskCode, conditions, statusFilter }: Partial<GetOrCreateTaskForCode>): Promise<Array<UUID> | null> {
		const connection = this.dbConnection;
		if (connection) {
			let query = db("integrations.data_business_integrations_tasks")
				.select("data_business_integrations_tasks.id")
			...
			if (statusFilter) {
				query = query.whereIn("data_business_integrations_tasks.task_status", statusFilter);
			} else {
				query = query.whereNotIn("data_business_integrations_tasks.task_status", ["SUCCESS", "IN_PROGRESS", "FAILED"]);
			}

			query = query.orderBy("data_business_integrations_tasks.created_at", "desc");
