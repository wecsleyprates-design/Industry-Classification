...
describe("AddressUtil", () => {
	...
	describe("UK address handling (FAST-273)", () => {
		it("should preserve full UK address when parse-address returns sparse results", () => {
			const ukAddress = "1 Saint Andrews House Vernon Gate, Derby, Derbyshire, DE1 1UJ, United Kingdom";
			const normalized = AddressUtil.normalizeString(ukAddress);
			expect(normalized).toBe(ukAddress);
		});

		it("should preserve UK address with abbreviated street name", () => {
			const ukAddress = "1 St Andrews House Vernon Gate, Derby, Derbyshire, DE1 1UJ";
			const normalized = AddressUtil.normalizeString(ukAddress);
			expect(normalized).toBe(ukAddress);
		});

		it("should not truncate UK address ending with GB country code", () => {
			const ukAddress = "1 St. Andrews House, Vernon Gate, Derby, Derby, DE1 1UJ, GB";
			const normalized = AddressUtil.normalizeString(ukAddress);
			expect(normalized).toContain("Andrews House");
			expect(normalized).toContain("Vernon Gate");
			expect(normalized).toContain("Derby");
			expect(normalized).toContain("GB");
		});

		it("should not truncate UK address with regional city name", () => {
			const ukAddress = "149 Victoria Road, Southend-On-Sea, Southend-On-Sea, SS1 2TD, GB";
			const normalized = AddressUtil.normalizeString(ukAddress);
			expect(normalized).toContain("149 Victoria Road");
			expect(normalized.toLowerCase()).toContain("southend-on-sea");
			expect(normalized).toContain("GB");
		});

		it("should still normalize US addresses correctly", () => {
			const usAddress = "123 Main Street, Apt 456, New York, NY 10001";
			const normalized = AddressUtil.normalizeString(usAddress);
			expect(normalized).toBe("123 Main St, Apt 456, New York, NY, 10001");
		});

		it("should produce non-empty result via stringToParts for UK addresses", () => {
			const ukAddress = "10 Downing Street, London, SW1A 2AA, United Kingdom";
			const parts = AddressUtil.stringToParts(ukAddress);
			expect(parts.line_1).toBeTruthy();
			expect(parts.formatted_address).toBeTruthy();
		});

		it("should never produce truncated UK addresses like '1 Saint' or '1 St'", () => {
			const ukAddresses = [
				"1 Saint Andrews House Vernon Gate, Derby, Derbyshire, DE1 1UJ, United Kingdom",
				"1 St Andrews House Vernon Gate, Derby, Derbyshire, DE1 1UJ",
				"1 St. Andrews House, Vernon Gate, Derby, Derby, DE1 1UJ, GB",
				"149 Victoria Road, Southend-On-Sea, Southend-On-Sea, SS1 2TD, GB"
			];
			for (const addr of ukAddresses) {
				const normalized = AddressUtil.normalizeString(addr);
