...
export class AddressUtil {
	...
	private static regexAddressParse(address: string): NormalizedAddress {
		...
		// If regex didn't match (common for non-US addresses like UK):
		// Preserve the original address in line_1 so that downstream callers
		// (normalizeString → partsToString) return the full address instead of "".
		logger.debug({
			message: "Fallback address parsing failed — preserving original address",
			addressLength: address.length
		});
		return {
			line_1: address,
	...
	public static stringToParts(address: string): NormalizedAddress {
		...
		// parse-address is a US-only parser. Non-US addresses (UK, etc.) typically
		// produce very sparse results (only {number, street}), silently discarding
		// city, state, and postal code. Fall through to the regex fallback when the
		// parsed result lacks all three key components.
		// Canadian addresses also produce sparse results but are fixed by the
		// post-parse Canadian handler below, so we must not short-circuit them.
		if (!parts || (!isCanadian && !parts.city && !parts.state && !parts.zip)) {
