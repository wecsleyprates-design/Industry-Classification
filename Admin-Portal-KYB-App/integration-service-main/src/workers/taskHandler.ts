...
export const initTaskWorker = () => {
	...
	taskQueue.queue.process(EVENTS.OWNERS_WATCHLIST_SUBMISSION, async (job: Job, done) => {
		await processJob(job, watchlistScreeningManager.ownersWatchlistSubmission.bind(watchlistScreeningManager));
		done();
	});
};
