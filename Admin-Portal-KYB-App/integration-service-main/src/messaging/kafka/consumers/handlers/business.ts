...
const useBq = async (): Promise<boolean> =>
	getFlagValue<boolean>(FEATURE_FLAGS.DEVOPS_118_BUSINESS_INVITED_ACCEPTED_BULL_QUEUE);
			...
			switch (event) {
				case kafkaEvents.BUSINESS_INVITE_ACCEPTED:
					...
					if (await useBq()) {
						...
						await businessOnboardingManager.seedBusinessIntegrations(payload);
					}
					break;
				...
				case kafkaEvents.OWNER_UPDATED:
					...
					if (await useBq()) {
						...
						await kafkaToQueue(taskQueue, EVENTS.OWNERS_WATCHLIST_SUBMISSION, payload, undefined, {
						delay: 120000,
						attempts: 6,
						...
						await Promise.allSettled([
							identityVerificationManager.plaidIdentityVerification(payload),
							creditBureauManager.fetchBureauCreditReport(payload),
							watchlistScreeningManager.ownersWatchlistSubmission(payload)
						]);
					}
					break;
				...
				case kafkaEvents.FETCH_ADVERSE_MEDIA_REPORT:
					...
					if (await useBq()) {
						...
						await adverseMediaManager.fetchAdverseMediaReport(payload);
					}
					break;
