...
jest.mock("#lib/match/matchUtil", () => ({
	MatchUtil: {
		...
		filterMatchResults: jest.fn(result => Promise.resolve(result))
	}
}));
...
describe("test report.ts", () => {
	...
	describe("_fetchAdverseMediaData", () => {
		...
		it("should fetch adverse media data by case ID when caseId is provided", async () => {
			...
			expect(adverseMedia.getAdverseMediaDataByCaseId).toHaveBeenCalledWith({ caseId: mockCaseId }, { sortFields: expectedSortFields });
	...
	describe("_extractArticleRows", () => {
		it("should extract articles from a valid AdverseMediaResponse", () => {
			const response = {
				articles: [
					{ id: "1", title: "Article 1", entity_focus_score: 9, risk_level: "HIGH", media_type: "business" },
					{ id: "2", title: "Article 2", entity_focus_score: 5, risk_level: "LOW", media_type: "business" }
				],
				total_risk_count: 2,
				high_risk_count: 1,
				medium_risk_count: 0,
			...
			const rows = reportEventsHandler._extractArticleRows(response as any);
			expect(rows).toHaveLength(2);
			expect(rows[0].title).toBe("Article 1");
		});

		it("should return empty array when articles is not an array", () => {
			const response = { total_risk_count: 0, high_risk_count: 0, medium_risk_count: 0, low_risk_count: 0, average_risk_score: 0 };

			const rows = reportEventsHandler._extractArticleRows(response as any);
			expect(rows).toEqual([]);
		});

		it("should return empty array when articles is undefined", () => {
			const response = { articles: undefined, total_risk_count: 0, high_risk_count: 0, medium_risk_count: 0, low_risk_count: 0, average_risk_score: 0 };
	...
	describe("_buildAdverseMediaSections", () => {
		it("should group articles by media_type, filter relevant, and calculate per-group risk levels", () => {
			const articles = [
				{ media_type: "business", risk_level: "HIGH", entity_focus_score: 9, title: "Article 1" },
				{ media_type: "business", risk_level: "LOW", entity_focus_score: 8, title: "Article 2" },
				{ media_type: "john doe", risk_level: "MEDIUM", entity_focus_score: 8, title: "Article 3" },
				{ media_type: "john doe", risk_level: "LOW", entity_focus_score: 9, title: "Article 4" }
			];

			const sections = reportEventsHandler._buildAdverseMediaSections(articles, "Test Corp");

			expect(sections).toHaveLength(2);

			const businessSection = sections.find(s => s.title === "TEST CORP");
			expect(businessSection).toBeDefined();
			expect(businessSection!.risk_level).toBe("HIGH");
			expect(businessSection!.relevant_articles).toHaveLength(1);
			expect(businessSection!.total_articles).toBe(2);

			const individualSection = sections.find(s => s.title === "John Doe");
			expect(individualSection).toBeDefined();
			expect(individualSection!.risk_level).toBe("MEDIUM");
			expect(individualSection!.relevant_articles).toHaveLength(1);
			expect(individualSection!.total_articles).toBe(2);
		});

		it("should use Title Case for business section when businessName is not available", () => {
			const articles = [
				{ media_type: "business", risk_level: "MEDIUM", entity_focus_score: 9, title: "Article 1" }
			];

			const sections = reportEventsHandler._buildAdverseMediaSections(articles, null);

			expect(sections[0].title).toBe("Business");
		});

		it("should set risk_level to LOW when no relevant articles exist in section", () => {
			const articles = [
				{ media_type: "business", risk_level: "LOW", entity_focus_score: 5, title: "Article 1" },
				{ media_type: "business", risk_level: "LOW", entity_focus_score: 3, title: "Article 2" }
			];

			const sections = reportEventsHandler._buildAdverseMediaSections(articles, "Corp");

			expect(sections[0].risk_level).toBe("LOW");
			expect(sections[0].relevant_articles).toHaveLength(0);
			expect(sections[0].total_articles).toBe(2);
		});

		it("should handle articles with missing media_type by defaulting to business", () => {
			const articles = [
				{ risk_level: "HIGH", entity_focus_score: 10, title: "Article 1" }
			];

			const sections = reportEventsHandler._buildAdverseMediaSections(articles, "Corp");

			expect(sections).toHaveLength(1);
			expect(sections[0].title).toBe("CORP");
			expect(sections[0].risk_level).toBe("HIGH");
		});

		it("should handle empty articles array", () => {
			const sections = reportEventsHandler._buildAdverseMediaSections([], "Corp");
			expect(sections).toHaveLength(0);
		});

		it("should filter out articles with entity_focus_score below 8", () => {
			const articles = [
				{ media_type: "business", risk_level: "HIGH", entity_focus_score: 10, title: "Relevant" },
				{ media_type: "business", risk_level: "HIGH", entity_focus_score: 7, title: "Not Relevant" },
				{ media_type: "business", risk_level: "MEDIUM", entity_focus_score: 8, title: "Borderline Relevant" },
				{ media_type: "business", risk_level: "MEDIUM", entity_focus_score: 5, title: "Low Score" }
			];

			const sections = reportEventsHandler._buildAdverseMediaSections(articles, "Corp");

			expect(sections[0].relevant_articles).toHaveLength(2);
			expect(sections[0].relevant_articles.map(a => a.title)).toEqual(["Relevant", "Borderline Relevant"]);
			expect(sections[0].total_articles).toBe(4);
		});

		it("should filter out LOW risk articles even with high entity_focus_score", () => {
			const articles = [
				{ media_type: "business", risk_level: "LOW", entity_focus_score: 10, title: "High Score Low Risk" },
				{ media_type: "business", risk_level: "HIGH", entity_focus_score: 9, title: "Relevant" }
			];

			const sections = reportEventsHandler._buildAdverseMediaSections(articles, "Corp");

			expect(sections[0].relevant_articles).toHaveLength(1);
			expect(sections[0].relevant_articles[0].title).toBe("Relevant");
			expect(sections[0].total_articles).toBe(2);
		});

		it("should cap relevant_articles at 10 per section", () => {
			...
			expect(sections[0].relevant_articles).toHaveLength(10);
			expect(sections[0].total_articles).toBe(15);
		});

		it("should preserve total_articles count from all articles in group regardless of relevance filter", () => {
			const articles = [
				{ media_type: "business", risk_level: "HIGH", entity_focus_score: 9, title: "Relevant 1" },
				{ media_type: "business", risk_level: "LOW", entity_focus_score: 3, title: "Not Relevant 1" },
				{ media_type: "business", risk_level: "MEDIUM", entity_focus_score: 2, title: "Not Relevant 2" },
				{ media_type: "business", risk_level: "HIGH", entity_focus_score: 8, title: "Relevant 2" }
			];
	...
	describe("_getMatchProData", () => {
		...
		beforeEach(() => {
			jest.clearAllMocks();
			MatchUtil.filterMatchResults.mockImplementation((result: any) => Promise.resolve(result));
		});
