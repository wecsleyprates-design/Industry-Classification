			...
			if (taskIds.length === 0) {
				const newTaskId = await platform.createTaskForCode({
					...
					metadata: {
						...
						prediction: payload.matches?.[0]?.prediction ?? null,
						integration_business: payload.matches?.[0]?.integration_business ?? {},
						source: payload.source
					}
				});
				logger.debug(`entityMatching: created new task ${newTaskId} for source ${payload.source}, business ${payload.business_id}`);
