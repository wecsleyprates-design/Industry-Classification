...
import { AdverseMediaResponse, SortField } from "#api/v1/modules/adverse-media/types";

const RELEVANCE_MIN_CONFIDENCE_SCORE = 8;
const MAX_ARTICLES_PER_SECTION = 10;

const DEFAULT_REPORT_SORT_FIELDS: SortField[] = [
...
class ReportEventsHandler {
	...
	async _getPublicRecordsData(body: I360Report): Promise<PublicRecordsResponse> {
		try {
			...
			if (adverseMediaData && Object.keys(adverseMediaData).length > 0) {
				const businessName = response.google_profile?.business_name !== "N/A"
					? response.google_profile?.business_name
					: null;
				const articles = this._extractArticleRows(adverseMediaData);
				const sections = this._buildAdverseMediaSections(articles, businessName);
				const totalDisplayed = sections.reduce((sum, s) => sum + s.relevant_articles.length, 0);

				response.adverse_media = {
					...adverseMediaData,
					sections,
					total_articles: articles.length,
					total_displayed_articles: totalDisplayed,
	...
	private async _fetchAdverseMediaData(businessId: UUID, caseId?: UUID) {
		try {
			if (caseId) {
				return await adverseMedia.getAdverseMediaDataByCaseId({ caseId }, { sortFields: DEFAULT_REPORT_SORT_FIELDS });
			} else {
				return await adverseMedia.getAdverseMediaByBusinessId({ businessId }, { sortFields: DEFAULT_REPORT_SORT_FIELDS });
			}
		} catch (error: any) {
	...
	/**
	 * Extracts article rows from an AdverseMediaResponse with proper type boundary handling.
	 * AdverseMediaResponseArticle declares camelCase fields, but fetchAdverseMediaData()
	 * returns raw DB rows with snake_case columns matching AdverseMediaArticleRow.
	 * This method isolates that type mismatch and guards for non-array data.
	 */
	private _extractArticleRows(response: AdverseMediaResponse): AdverseMediaArticleRow[] {
		if (!Array.isArray(response.articles)) return [];
		return response.articles as unknown as AdverseMediaArticleRow[];
	}

	private _buildAdverseMediaSections(articles: AdverseMediaArticleRow[], businessName?: string | null): AdverseMediaSection[] {
		const grouped = Object.groupBy(articles, (article) => article.media_type || "business");

		return Object.entries(grouped).map(([mediaType, groupArticles]) =>
			this._buildSectionFromArticles(mediaType, groupArticles!, businessName)
		);
	}

	private _buildSectionFromArticles(
		mediaType: string,
		groupArticles: AdverseMediaArticleRow[],
		businessName?: string | null
	): AdverseMediaSection {
		// Articles arrive pre-sorted by DEFAULT_REPORT_SORT_FIELDS
		// (risk_level HIGH first, then entity_focus_score DESC, final_score DESC),
		// so the first qualifying entries are already the highest-priority ones.
		const relevant_articles: AdverseMediaArticleRow[] = [];
		for (const a of groupArticles) {
			if (relevant_articles.length >= MAX_ARTICLES_PER_SECTION) break;
			if (a.entity_focus_score >= RELEVANCE_MIN_CONFIDENCE_SCORE && (a.risk_level === "MEDIUM" || a.risk_level === "HIGH")) {
				relevant_articles.push(a);
			}
		}

		const hasHigh = relevant_articles.some(a => a.risk_level === "HIGH");
		const hasMedium = relevant_articles.some(a => a.risk_level === "MEDIUM");
		const riskLevel: AdverseMediaSection["risk_level"] = hasHigh ? "HIGH" : hasMedium ? "MEDIUM" : "LOW";

		const title = mediaType === "business" && businessName
			? businessName.toUpperCase()
			: mediaType.split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
	...
	async _getMatchProData(body: I360Report): Promise<MatchProData | {}> {
		...
		try {
			...
			if (isMultiIcaMatchResponse(response)) {
				...
				const filteredResult = await MatchUtil.filterMatchResults(defaultResult);

				return {
					icas: [defaultIcaEntry],
					results: hasErrors ? {} : { [defaultIca]: filteredResult },
			...
			const legacyResult = await MatchUtil.filterMatchResults({ terminationInquiryRequest: response.terminationInquiryRequest, terminationInquiryResponse: response.terminationInquiryResponse });
			return {
				icas: [{ ica: legacyIca, isDefault: true }],
				results: {
					[legacyIca]: legacyResult,
				},
				multi_ica: false,
				timestamp: response.timestamp
			};
