...
export const getBusinessDetails = async (
	...
	/** When true, adds customer_org_sandbox=true for case-service internal TIN gating (caller asserts sandbox org from auth). */
	customerOrgIsSandbox?: boolean
): Promise<BusinessDetailsResponse> => {
	try {
		const sandboxQuery = customerOrgIsSandbox === true ? "&customer_org_sandbox=true" : "";
		const config: AxiosRequestConfig = {
			method: "get",
			url: `${envConfig.CASE_BASE_URL}/api/v1/internal/businesses/${businessID}?fetch_owner_details=true${tinBehavior ? "&tinBehavior=" + tinBehavior : ""}${sandboxQuery}`,
			headers: {
