...
import { isTruliooCompletedStatus, extractFieldFromTruliooServiceData, extractStandardizedLocationsFromTruliooResponse, extractIncorporationDateFromTrulioo , convertToUUIDFormat ,
	...
	extractRecordStatusFromTruliooResponse,
...
export class TruliooBusinessResultsStorage {
	...
	async storeBusinessVerificationResults(
		...
		try {
			...
			if (!shouldScreen) {
				...
				await this.resolveOrphanedPSCTask(`PSC screening not applicable: ${reason}`);
				return;
			}
			...
			await this.resolveOrphanedPSCTask("PSC screening completed inline during KYB flow");
		}
		} catch (error: unknown) {
	...
	private async storeAddressData(
		...
		try {
			...
			const clientDataRecord = clientData as Record<string, unknown> | undefined;

			// Derive deliverable from Trulioo signals when status is definitive.
			// UK/international responses lack the Middesk-style deliverable field, so we
			// infer it from DatasourceFields or RecordStatus.
			const addressMatchResult = this.isDefinitiveResult(clientDataStatus)
				? this.resolveAddressMatchStatus(clientDataRecord)
				: undefined;
			const addressDeliverable = addressMatchResult === "match" ? true
				: addressMatchResult === "nomatch" ? false
				: undefined;
			...
			if (!existingSubmitted && (businessPayload.companyAddressFull || businessPayload.companyCity)) {
				...
				if (record) {
					if (addressDeliverable !== undefined) record.deliverable = addressDeliverable;
					records.push(record);
				}
			}

			// Reported addresses from serviceData (StandardizedLocations)
			const reported = clientDataRecord
				? extractStandardizedLocationsFromTruliooResponse(clientDataRecord)
				: undefined;
			...
			if (records.length > 0) {
				...
				.merge({
					...
					deliverable: db.raw("COALESCE(EXCLUDED.deliverable, integration_data.business_entity_address_source.deliverable)")
				})
					.returning("*");
			...
			} else if (existingSubmitted && addressDeliverable !== undefined && existingSubmitted.deliverable === null) {
				await db<IBusinessEntityAddressSource>("integration_data.business_entity_address_source")
					.where({ business_entity_verification_id: businessEntityVerificationId })
					.whereNull("deliverable")
					.update({ deliverable: addressDeliverable });
				logger.info(`Back-filled deliverable=${addressDeliverable} for address sources on verification ${businessEntityVerificationId}`);
			}
	...
	/**
	 * Resolve the address match status from Trulioo signals using a priority cascade:
	 *   1. DatasourceFields (Comprehensive View or all datasources)
	 *   2. Top-level RecordStatus (UK/international responses have empty DatasourceFields
	 *      but a valid RecordStatus)
	 *
	 * Used by both address deliverable derivation and review task creation to avoid
	 * duplicating the resolution logic.
	 */
	private resolveAddressMatchStatus(
		clientDataRecord: Record<string, unknown> | undefined
	): "match" | "nomatch" | undefined {
		const dsMatch = extractAddressMatchStatusFromDatasourceFields(clientDataRecord);
		if (dsMatch === "match" || dsMatch === "nomatch") return dsMatch;

		return extractRecordStatusFromTruliooResponse(clientDataRecord);
	}

	/**
	 * Determine and create the address_verification review task based on Trulioo signals.
	 *
	 * Resolution order:
	 *   1. DatasourceFields / RecordStatus → direct match/nomatch signal
	 *   2. Sibling verification task → reuse result from another verification of the same business
	 *   3. Overall clientData status → last resort fallback
	 *
	...
	private async resolveAddressVerificationTask(
		...
		try {
			// Priority 1: DatasourceFields or RecordStatus signal
			const addressMatchStatus = this.resolveAddressMatchStatus(clientDataRecord);

			if (addressMatchStatus === "match" || addressMatchStatus === "nomatch") {
				const taskStatus = addressMatchStatus === "match" ? "success" : "failure";
				const sublabel = taskStatus === "success" ? "Verified" : "Unverified";
				logger.info(`Address verification for ${businessEntityVerificationId}: Trulioo signal → "${addressMatchStatus}" — marking as ${sublabel}`);
				await this.createAddressVerificationTask(businessEntityVerificationId, taskStatus, sublabel);
				return;
			}

			// Priority 2: No direct signal (e.g. PSC screening without KYB data).
			...
			if (siblingTask && (siblingTask.status === "success" || siblingTask.status === "failure")) {
				logger.info(`Address verification for ${businessEntityVerificationId}: reusing sibling task (status="${siblingTask.status}", sublabel="${siblingTask.sublabel}")`);
			...
			// Priority 3: Last resort — derive from overall Trulioo status
			const taskStatus = this.deriveTaskStatus(clientDataStatus);
			const sublabel = taskStatus === "success" ? "Verified" : "Unverified";
			logger.info(`Address verification for ${businessEntityVerificationId}: no signal/sibling, falling back to overall status "${clientDataStatus}" → ${sublabel}`);
			await this.createAddressVerificationTask(businessEntityVerificationId, taskStatus, sublabel);
		} catch (error) {
			logger.error(
				{ err: error, businessEntityVerificationId, clientDataStatus },
				`Failed to resolve address verification task for ${businessEntityVerificationId}`
			);
		}
	}
	...
	/**
	 * Resolve platform 42 (trulioo_psc) tasks that were seeded for non-US businesses.
	 *
	 * Platform 42 tasks are only useful for US businesses (deferrable PSC via Middesk).
	 * For non-US businesses, PSC screening runs inline during KYB. This method marks
	 * orphaned platform 42 tasks as SKIPPED so they don't remain CREATED forever.
	 */
	private async resolveOrphanedPSCTask(reason: string): Promise<void> {
		try {
			const businessId = this.truliooBase["businessID"];

			const tasksToResolve = await db("integrations.data_business_integrations_tasks as dbit")
				.join("integrations.data_connections as dc", "dc.id", "dbit.connection_id")
				.join("integrations.rel_tasks_integrations as rti", "rti.id", "dbit.integration_task_id")
				.join("integrations.core_tasks as ct", "ct.id", "rti.task_category_id")
				.where("dc.business_id", businessId)
				.where("dc.platform_id", INTEGRATION_ID.TRULIOO_PSC)
				.where("ct.code", "fetch_watchlist_hits")
				.where("dbit.task_status", "CREATED")
				.select("dbit.id");
			...
			for (const task of tasksToResolve) {
				await db.transaction(async (trx) => {
					const updated = await trx("integrations.data_business_integrations_tasks")
						.where("id", task.id)
						.andWhere("task_status", "CREATED")
						.update({
							task_status: "SKIPPED",
					...
					if (updated > 0) {
						await trx("integrations.business_integration_tasks_events")
							.insert({
								id: randomUUID(),
								business_integration_task_id: task.id,
								task_status: "SKIPPED",
								log: JSON.stringify({ log: reason })
							});
					}
				});
				...
				logger.info(
					...
					`Resolved orphaned PSC task as SKIPPED: ${reason}`
				);
			}
		} catch (error) {
			logger.warn(
				...
				"Failed to resolve orphaned PSC task (non-critical)"
			);
		}
	}
