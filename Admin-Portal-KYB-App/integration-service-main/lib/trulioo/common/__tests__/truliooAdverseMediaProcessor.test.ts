import { processAndPersistTruliooAdverseMedia, extractTitleFromUrl, extractSourceFromUrl, toTitleCase } from "../truliooAdverseMediaProcessor";
...
describe("processAndPersistTruliooAdverseMedia", () => {
	...
	describe("filtering", () => {
		...
		it("should filter case-insensitively (lowercase adverse_media)", async () => {
			const hits = [
				createHit({ listType: "adverse_media" as any, listName: "Lowercase Hit", url: "https://example.com/lowercase-hit-article" }),
				createHit({ listType: "Adverse_Media" as any, listName: "Mixed Case Hit", url: "https://example.com/mixed-case-hit-article" })
			];
	...
	describe("URL title/source enrichment", () => {
		describe("toTitleCase", () => {
			it("should capitalize each word except minor words", () => {
				expect(toTitleCase("woman allegedly tried to fly with gift wrapped heroin"))
					.toBe("Woman Allegedly Tried to Fly with Gift Wrapped Heroin");
			});

			it("should always capitalize the first word even if it is a minor word", () => {
		...
		describe("extractTitleFromUrl", () => {
			it("should extract readable title from URL slug with Title Case", () => {
		...
		describe("extractSourceFromUrl", () => {
			it("should extract formatted publication name from domain", () => {
				expect(extractSourceFromUrl("https://www.cbsnews.com/some/path")).toBe("Cbsnews");
			});

			it("should handle .co.uk TLD", () => {
				expect(extractSourceFromUrl("https://www.belfasttelegraph.co.uk/news")).toBe("Belfasttelegraph");
			});

			it("should strip .com TLD and apply Title Case", () => {
				expect(extractSourceFromUrl("https://patch.com/new-jersey/news")).toBe("Patch");
			});

			it("should strip .org TLD", () => {
				expect(extractSourceFromUrl("https://www.ipsnews.org/article")).toBe("Ipsnews");
			});
		...
		describe("integration with processor", () => {
			it("should use URL slug as title with Title Case when listName is generic 'Adverse Media'", async () => {
				const hits: TruliooWatchlistHit[] = [
					createHit({
			...
			it("should use formatted domain as source when sourceAgencyName is not provided", async () => {
				const hits: TruliooWatchlistHit[] = [
					createHit({
						listName: "Adverse Media",
				...
				const insertedData = mockInsertAdverseMedia.mock.calls[0][2];
				expect(insertedData.articles[0].source).toBe("Cbsnews");
			});

			it("should set date to null since Trulioo does not provide article publish dates", async () => {
				const hits: TruliooWatchlistHit[] = [
					createHit({
						listName: "News Article",
						url: "https://example.com/news-article"
					})
				];

				mockScoreAdverseMedia.mockResolvedValue({
					finalScore: 5, riskLevel: "MEDIUM", description: "test", mediaType: "business"
				});
				mockInsertAdverseMedia.mockResolvedValue(undefined);
				...
				const insertedData = mockInsertAdverseMedia.mock.calls[0][2];
				expect(insertedData.articles[0].date).toBeNull();
			});
