 ...
 *
 * NOTE: StateProvinceCode is intentionally excluded. Trulioo returns
 * the full region name (e.g. "Derby", "Glasgow City", "Southend-On-Sea")
 * while customers typically submit abbreviated codes ("DBY", "SCT", "ESS").
 * This mismatch produces false "nomatch" results across ALL datasources
 * (Comprehensive View, Business Insights, etc.) for UK and other
 * international businesses. City + PostalCode are sufficient to validate
 * the geographic component of an address.
 */
...
/** Minimal shape of a Trulioo serviceData item for address field extraction. */
interface TruliooServiceDataItem {
	fullServiceDetails?: {
		Record?: {
...
export function extractAddressMatchStatusFromDatasourceFields(
	...
	// Fallback: check all datasources. With StateProvinceCode excluded from
	// ADDRESS_FIELD_NAMES, Business Insights fields (City, PostalCode, etc.)
	// can safely detect genuinely mismatched addresses without false nomatch
	// from abbreviated UK state codes.
...
function evaluateAddressFields(
	...
	for (const serviceDataItem of serviceDataArray) {
		const item = serviceDataItem as TruliooServiceDataItem;
		const datasourceResults = item?.fullServiceDetails?.Record?.DatasourceResults;
		if (!Array.isArray(datasourceResults)) continue;
		...
		for (const datasource of datasourceResults) {
			...
			for (const field of fields) {
				if (!field?.FieldName || !ADDRESS_FIELD_NAMES.has(field.FieldName)) continue;
...
/**
 * Extracts the overall RecordStatus from the Trulioo KYB response.
 *
 * UK/international Trulioo responses often return empty DatasourceFields
 * (no per-field match/nomatch status), but the top-level Record.RecordStatus
 * still indicates whether the business data matched: "match" or "nomatch".
 *
 * This is useful as a secondary fallback when DatasourceFields-based
 * address verification returns undefined.
 *
 * @param clientData - The clientData from Trulioo response
 * @returns "match" | "nomatch" | undefined
 */
export function extractRecordStatusFromTruliooResponse(
	clientData: Record<string, unknown> | undefined
	...
	// Scan all records with deterministic precedence: nomatch is dominant
	// (aligned with evaluateAddressFields where any nomatch → overall nomatch).
	let foundMatch = false;

	for (const serviceDataItem of serviceDataArray) {
		const item = serviceDataItem as TruliooServiceDataItem;
		const recordStatus = item?.fullServiceDetails?.Record?.RecordStatus;
		if (typeof recordStatus === "string") {
			const normalized = recordStatus.toLowerCase();
			if (normalized === "nomatch") return "nomatch";
			if (normalized === "match") foundMatch = true;
		}
	}

	return foundMatch ? "match" : undefined;
}
