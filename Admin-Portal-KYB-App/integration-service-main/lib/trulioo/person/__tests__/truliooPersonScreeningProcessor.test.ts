...
describe("TruliooPersonScreeningProcessor", () => {
	...
	describe("URL and source field propagation", () => {
		it("should propagate url, sourceAgencyName, and sourceRegion from raw hits", async () => {
			const mockFlowResult: TruliooFlowResult = {
				hfSession: "session-url",
				external_id: "external-url",
				status: "completed",
				flowData: { id: "flow-url" },
				clientData: {
					status: "COMPLETED",
					watchlistResults: [
						{
							...
							confidence: 0.9,
							matchDetails: "Media match",
							url: "https://www.cbsnews.com/news/some-article/",
							sourceAgencyName: "CBS News",
							sourceRegion: "US"
						},
						{
							listType: "SANCTIONS",
							...
							url: "https://ofac.treasury.gov/entity/12345",
			...
			expect(result.watchlistHits).toHaveLength(2);
			expect(result.watchlistHits[0]).toEqual(expect.objectContaining({
				listType: "ADVERSE_MEDIA",
				url: "https://www.cbsnews.com/news/some-article/",
				sourceAgencyName: "CBS News",
				sourceRegion: "US"
			}));
			expect(result.watchlistHits[1]).toEqual(expect.objectContaining({
				listType: "SANCTIONS",
				url: "https://ofac.treasury.gov/entity/12345",
				sourceAgencyName: "OFAC"
			}));
		...
		it("should set url/sourceAgencyName/sourceRegion to undefined when not present in raw hit", async () => {
			const mockFlowResult: TruliooFlowResult = {
				hfSession: "session-no-url",
				external_id: "external-no-url",
				status: "completed",
				flowData: { id: "flow-no-url" },
				clientData: {
					status: "COMPLETED",
					watchlistResults: [
						{
			...
			expect(result.watchlistHits[0].url).toBeUndefined();
			expect(result.watchlistHits[0].sourceAgencyName).toBeUndefined();
			expect(result.watchlistHits[0].sourceRegion).toBeUndefined();
		});
	});
