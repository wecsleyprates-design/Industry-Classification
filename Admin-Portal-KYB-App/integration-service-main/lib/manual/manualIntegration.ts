...
import { CaseServiceClient } from "#clients/case/caseServiceClient";
...
export class ManualIntegration extends TaskManager {
	 ...
	 * Behavior:
	 * - PATCH: (merge) returns the current fact overrides with the provided overrides added or updated (provided overrides are merged with current overrides).
	 * - PUT: (replace) returns a state that contains only the provided overrides (provided overrides replace current overrides).
	 * - DELETE: (remove) returns the remaining fact overrides after removing any whose keys are present in the provided overrides (provided overrides are removed from current overrides).
	 */
	private static computeNextFactOverrideState(
		...
		if (method === "PATCH" || method === "PUT") {
			...
			return Object.entries(providedOverrides).reduce((acc, [factName, override]) => {
			...
			if (newAccountCreated && giactTaskIdToUse) {
				...
				if (giact) {
					try {
						...
						logger.error(
							{ businessID: event.business_id, error: (err as Error).message },
							"Error processing GIACT verification"
						);
	...
	processProcessingHistoryRecords = async (event: IntegrationDataUploadedEventType) => {
		...
		try {
			if (event?.data) {
				...
				const hasRelevantKey = Object.entries(eventData).some(([key, value]) => relevantKeys.has(key) && value !== null && value !== undefined);
				if (hasRelevantKey) {
					const payload = {
						case_id: event.case_id,
						general_data: {
	...
	public async updateFactOverride(
		...
		await Promise.all([
			this.saveRawResponseToDB({ ...event, data: newFactOverrides }, taskLike, "fact_override"),
			...
			sendWebhookEvent(event.customer_id, WEBHOOK_EVENTS.FACT_OVERRIDE_UPDATED, {
				business_id: event.business_id,
				case_id: event.case_id,
				method: contextParams.method,
				user_id: event.user_id,
				updated_at:
					event.created_at instanceof Date ? event.created_at.toISOString() : new Date(event.created_at).toISOString(),
				/** Calculate just the fact overrides that were changed and exclude any current fact overrides. */
				data: factNames.reduce((acc, factName) => {
					if (newFactOverrides[factName]) acc[factName] = newFactOverrides[factName];
					return acc;
				}, {})
			})
		]);
		...
		await ManualIntegration.syncCaseBusinessDisplayProfileFromFactOverrides({
			method: contextParams.method,
			businessID,
	...
	/**
	 * After KYB fact overrides for `legal_name` / `dba`, update case-service business display fields
	 * so case overview and listings stay in sync. Non-blocking for the override transaction.
	 */
	private static async syncCaseBusinessDisplayProfileFromFactOverrides(params: {
		method: "DELETE" | "PATCH" | "PUT";
		businessID: UUID;
		userID: UUID;
		record: Partial<Record<FactName, FactOverride | Pick<FactOverride, "value" | "comment">>>;
		...
		const { method, businessID, userID, record, newFactOverrides } = params;
		if (method === "DELETE") {
			return;
		}

		const hasLegalKey = Object.prototype.hasOwnProperty.call(record, "legal_name");
		const hasDbaKey = Object.prototype.hasOwnProperty.call(record, "dba");
		if (!hasLegalKey && !hasDbaKey) {
			return;
		}

		const body: { userId: UUID; name?: string; dba_names?: { name: string }[] } = {
			userId: userID
		};

		if (hasLegalKey) {
			const v = newFactOverrides.legal_name?.value;
		...
		if (hasDbaKey) {
			const v = newFactOverrides.dba?.value;
			if (v === null || v === undefined) {
				...
				body.dba_names = v.trim() ? [{ name: v.trim() }] : [];
			} else {
				body.dba_names = [];
			}
		}

		if (body.name === undefined && body.dba_names === undefined) {
			return;
		}

		const caseClient = new CaseServiceClient();
		await caseClient.patchBusinessDisplayProfile(businessID, body);
	}
