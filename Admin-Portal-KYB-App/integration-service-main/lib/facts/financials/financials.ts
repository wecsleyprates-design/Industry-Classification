...
const simpleFacts: SimpleFact = {
	revenue: {
		accountingIncomeStatements: async (_, accountingIncomeStatements: ReportResponse): Promise<number | undefined> => {
			const value = await extractFromAccountingIncomeStatements(accountingIncomeStatements, "total_revenue");
			if (value === null || value === 0) return undefined;
			return value;
		},
		equifax: async (_, equifax: IEquifaxJudgementsLiens): Promise<number | undefined> => {
			const corpAmount = equifax?.scoring_model?.corpamount;
			return typeof corpAmount === "number" ? corpAmount * 1000 : undefined;
		},
		manual: "is_revenue",
