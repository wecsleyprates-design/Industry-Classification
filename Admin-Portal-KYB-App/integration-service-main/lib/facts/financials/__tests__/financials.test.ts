...
describe("financial facts", () => {
	describe("revenue", () => {
		...
		it("should not return accounting revenue when total_revenue is 0 (treated as unreliable)", async () => {
			/** Arrange */
			const accountingResponse = {
				accounting_incomestatement: {
					"2023": {
		...
		it("should select equifax revenue when accounting returns empty data", async () => {
			/** Arrange */
			sources.accountingIncomeStatements.getter = jest.fn().mockResolvedValue({});
			sources.accountingIncomeStatements.confidence = undefined as any;
			...
			sources.equifax.getter = jest.fn().mockResolvedValue(equifaxResponse);
			sources.equifax.confidence = 0.8;
		...
		it("should select zoominfo revenue when accounting returns $0 revenue", async () => {
			/** Arrange */
			const accountingResponse = {
				accounting_incomestatement: {
					"2023": {
			...
			sources.accountingIncomeStatements.getter = jest.fn().mockResolvedValue(accountingResponse);
			sources.accountingIncomeStatements.confidence = 0.99;

			const zoominfoResponse = {
				firmographic: {
					zi_c_company_revenue: 1200,
					zi_c_company_revenue_confidence: 0.9
				}
			};
			sources.zoominfo.getter = jest.fn().mockResolvedValue(zoominfoResponse);
			sources.zoominfo.confidence = 0.5;
		...
		it("should select accounting revenue when it has real non-zero data with high confidence", async () => {
			/** Arrange */
			const accountingResponse = {
				accounting_incomestatement: {
					"2023": {
			...
			sources.equifax.getter = jest.fn().mockResolvedValue(equifaxResponse);
			sources.equifax.confidence = 0.7;
		...
		it("should not return equifax revenue when corpamount is null (explicit null check)", async () => {
			/** Arrange */
			const equifaxResponse = {
				scoring_model: {
					corpamount: null
		...
		it("should fall back to zoominfo when accounting returns null total_revenue", async () => {
			/** Arrange */
			const accountingResponse = {
				accounting_incomestatement: {
					"2023": {
			...
			const zoominfoResponse = {
				firmographic: {
					zi_c_company_revenue: 750,
					zi_c_company_revenue_confidence: 0.85
				}
			};
			sources.zoominfo.getter = jest.fn().mockResolvedValue(zoominfoResponse);
			sources.zoominfo.confidence = 0.4;
			...
			/** Assert */
			expect(r?.value).toBe(750000);
		});

		it("should fall back to equifax when accounting_incomestatement is an empty object", async () => {
			/** Arrange */
			const accountingResponse = {
				accounting_incomestatement: {}
			};
			...
			sources.equifax.getter = jest.fn().mockResolvedValue(equifaxResponse);
			sources.equifax.confidence = 0.6;
		...
		it("should not return equifax revenue when corpamount is 0", async () => {
			/** Arrange */
			const equifaxResponse = {
				scoring_model: {
					corpamount: 0
			...
			/** Assert - corpamount 0 * 1000 = 0, which is valid per FactEngine but still a real value */
