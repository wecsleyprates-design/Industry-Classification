...
const dictionary = {
	...
	accountingIncomeStatements: {
		...
		category: "accounting",
		scope: "business",
		getter: async function (business_id: any): Promise<ReportResponse> {
			const report = await AccountingRest.getReport({
				business_id,
				report: "accounting_incomestatement",
				params: { groupBy: "year" }
			});
			if (report?.accounting_incomestatement && Object.keys(report.accounting_incomestatement).length > 0) {
	...
	accountingCashFlows: {
		...
		scope: "business",
		getter: async function (business_id: any): Promise<ReportResponse> {
			const report = await AccountingRest.getReport({ business_id, report: "accounting_cashflow", params: { groupBy: "year" } });
			if (report?.accounting_cashflow && Object.keys(report.accounting_cashflow).length > 0) {
				this.confidence = 0.99;
			}
			return report;
		}
	...
	accountingBalanceSheetsS3: {
		...
		category: "accounting",
		scope: "business",
		getter: async function (business_id: any): Promise<any> {
	...
	accountingBalanceSheetsCase: {
		...
		category: "accounting",
		scope: "case",
		getter: async function (case_id: any) {
			const report = await AccountingRest.getReport({ case_id, report: "accounting_balancesheet", params: { groupBy: "year" } });
			if (report?.accounting_balancesheet && Object.keys(report.accounting_balancesheet).length > 0) {
				this.confidence = 0.99;
			}
			return report;
		}
	...
	accountingIncomeStatementsCase: {
		...
		scope: "case",
		getter: async function (case_id: any) {
			const report = await AccountingRest.getReport({ case_id, report: "accounting_incomestatement", params: { groupBy: "year" } });
			if (report?.accounting_incomestatement && Object.keys(report.accounting_incomestatement).length > 0) {
				this.confidence = 0.99;
			}
			return report;
	...
	accountingCashFlowsCase: {
		...
		scope: "case",
		getter: async function (case_id: any) {
			const report = await AccountingRest.getReport({ case_id, report: "accounting_cashflow", params: { groupBy: "year" } });
			if (report?.accounting_cashflow && Object.keys(report.accounting_cashflow).length > 0) {
				this.confidence = 0.99;
			}
			return report;
