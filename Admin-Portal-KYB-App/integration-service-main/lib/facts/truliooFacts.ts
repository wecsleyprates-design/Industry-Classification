...
export const truliooFacts: readonly Fact[] = [
...

	{
		...
		fn: async (_, truliooPersonResponse: TruliooPersonSourceResponse | undefined): Promise<TruliooScreenedPersonData[]> => {
			const people = truliooPersonResponse?.screenedPersons || [];
			const isNotAdverseMedia = (hit: TruliooWatchlistHit) =>
				(hit.listType ?? "").toUpperCase() !== "ADVERSE_MEDIA";

			// Strip adverse media hits from both canonical (screeningResults.watchlistHits)
			// and legacy (watchlistResults) locations. Adverse media is served exclusively
			// by the adverse media pipeline (adverse_media_articles table).
			return people.map(person => ({
				...person,
				watchlistResults: Array.isArray(person.watchlistResults)
					? person.watchlistResults.filter(isNotAdverseMedia)
					: person.watchlistResults,
				screeningResults: {
					...person.screeningResults,
					watchlistHits: (person.screeningResults?.watchlistHits || []).filter(isNotAdverseMedia)
				}
			}));
		},
		description: "People screened by PSC flow (adverse media hits excluded)"
	},
		...
		fn: async (_, truliooPersonResponse: TruliooPersonSourceResponse | undefined): Promise<TruliooWatchlistHit[]> => {
			...
			truliooPersonResponse?.screenedPersons?.forEach((person: TruliooScreenedPersonData) => {
				...
				...person.screeningResults.watchlistHits.filter(
						(hit: TruliooWatchlistHit) => (hit.listType ?? "").toUpperCase() === "ADVERSE_MEDIA"
					)
					);
				}
			});
