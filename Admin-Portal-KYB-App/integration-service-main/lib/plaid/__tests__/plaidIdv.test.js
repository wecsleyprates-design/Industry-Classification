...
describe("PlaidIdv", () => {
	...
	describe("buildDocumentIntegrityResults (KYC owners API idv.document_integrity_results)", () => {
		it("returns expected format from Plaid documentary_verification meta", () => {
			const meta = {
				documentary_verification: {
					documents: [
						{
							...
							analysis: {
								authenticity: "match",
								image_quality: "high",
								aamva_verification: { is_verified: true }
							}
						}
					]
				}
			};
			const result = PlaidIdv.buildDocumentIntegrityResults(meta);
			...
			expect(result[0]).toMatchObject({
				...
				details: {
					authenticity: "match",
					image_quality: "high",
					aamva_is_verified: true
				}
			});
			expect(result[0].details).toHaveProperty("authenticity");
			expect(result[0].details).toHaveProperty("image_quality");
			expect(result[0].details).toHaveProperty("aamva_is_verified");
		});

		it("returns empty array when meta has no documentary_verification", () => {
			expect(PlaidIdv.buildDocumentIntegrityResults({})).toEqual([]);
			expect(PlaidIdv.buildDocumentIntegrityResults(null)).toEqual([]);
			expect(PlaidIdv.buildDocumentIntegrityResults(undefined)).toEqual([]);
		});

		it("returns empty array when meta has documentary_verification but documents is empty", () => {
			expect(PlaidIdv.buildDocumentIntegrityResults({ documentary_verification: {} })).toEqual([]);
			expect(PlaidIdv.buildDocumentIntegrityResults({ documentary_verification: { documents: [] } })).toEqual([]);
		});

		it("returns one result per document when meta has more than one document", () => {
			const meta = {
				documentary_verification: {
					documents: [
						{
							attempt: 1,
							status: "success",
							analysis: { authenticity: "match", image_quality: "high", aamva_verification: { is_verified: true } }
						},
						{
							attempt: 2,
							status: "failed",
							analysis: { authenticity: "no_match", image_quality: "low", aamva_verification: null }
						},
						{
							attempt: 3,
							status: "manually_approved",
							analysis: { authenticity: "match", image_quality: "medium", aamva_verification: { is_verified: false } }
						}
					]
				}
			};
			...
			expect(result[1]).toMatchObject({
				attempt: 2,
				plaid_document_status: "failed",
				document_integrity_status: "failed"
			});
			expect(result[1].details).toMatchObject({
				authenticity: "no_match",
				image_quality: "low",
				aamva_is_verified: null
			});
			expect(result[2]).toMatchObject({
				attempt: 3,
				plaid_document_status: "manually_approved",
				document_integrity_status: "clear"
			});
			expect(result[2].details).toMatchObject({
				authenticity: "match",
				image_quality: "medium",
				aamva_is_verified: false
			});
		});

		it("handles object-shaped documents input and preserves expected ordering", () => {
			const meta = {
				documentary_verification: {
					documents: {
						first: {
		...
		it("maps document status to document_integrity_status (clear, failed, suspected)", () => {
			const meta = {
				documentary_verification: {
					documents: [
						{ attempt: 1, status: "manually_approved", analysis: {} },
						{ attempt: 2, status: "failed", analysis: {} },
						{ attempt: 3, status: "pending", analysis: {} }
					]
				}
			};
			const result = PlaidIdv.buildDocumentIntegrityResults(meta);
			expect(result[0].document_integrity_status).toBe("clear");
			expect(result[1].document_integrity_status).toBe("failed");
			expect(result[2].document_integrity_status).toBe("suspected");
		});
	});
});
