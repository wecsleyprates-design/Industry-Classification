...
import { applySandboxIdvPersonaOverride } from "./util/applySandboxIdvPersonaOverride";
...
export class PlaidIdv extends TaskManager {
	...
	async getIdvStatusFromPlaid(
		...
		if (connection) {
			...
			if (result && result.data) {
				...
				if (this.strategyConfig.plaidEnv === "sandbox") {
					result.data = applySandboxIdvPersonaOverride(result.data);
				}
	...
	//  Builds document integrity results from Plaid IDV meta (documentary_verification.documents).
	private static buildDocumentIntegrityResults(meta: unknown): IPlaidIDV.DocumentIntegrityResults {
		const docVerification = (meta as Record<string, unknown> | null | undefined)?.documentary_verification as
			| { documents?: IPlaidIDV.DocumentaryVerificationDocument[] | Record<string, IPlaidIDV.DocumentaryVerificationDocument> }
			| undefined;
		const docs = docVerification?.documents;
		const docArray: IPlaidIDV.DocumentaryVerificationDocument[] = docs
			? (Array.isArray(docs) ? docs : Object.values(docs))
			: [];
		const sortedDocs = docArray.slice().sort((a, b) => {
			return (a.attempt ?? Number.MAX_SAFE_INTEGER) - (b.attempt ?? Number.MAX_SAFE_INTEGER);
		});
		return sortedDocs.map(doc => {
			const status = doc.status;
			const document_integrity_status: "clear" | "suspected" | "failed" =
				status === "success" || status === "manually_approved"
					? "clear"
					: status === "failed"
						? "failed"
						: "suspected";
			return {
				...
				plaid_document_status: status ?? "pending",
				document_integrity_status,
				details: {
					authenticity: doc.analysis?.authenticity ?? null,
					image_quality: doc.analysis?.image_quality ?? null,
					aamva_is_verified: doc.analysis?.aamva_verification?.is_verified ?? null
				}
			};
		});
	}
	...
	static async getVerificationForBusinessOwners(businessID: UUID) {
		...
		for (let i = 0; i < owners.length; i++) {
			...
			if (record && record.meta?.kyc_check) {
				...
				owners[i].idv = {
					document_integrity_results: PlaidIdv.buildDocumentIntegrityResults(record.meta)
				};
			} else {
