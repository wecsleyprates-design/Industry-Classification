...
import { verifyAndFormatNationalNumber } from "#utils";
...
import {
	...
	getCustomerOrgSandboxFlagForCaseBusinessDetails,
	remapBusinessFeinForSandboxGiact,
	resolveGiactStrategyModeForCustomer
} from "./sandbox";
...
export class GIACT extends TaskManager {
	...
	public static async initializeGiactConnection(businessID: UUID, customerID?: UUID): Promise<GIACT> {
		...
		logger.info(
			`GIACT feature flag check: useStrategy=${useStrategy}, businessID=${businessID}, customerID=${customerID ?? "none"}`
		);
		if(useStrategy) {
	...
	public static async initializeGiactConnectionWithStrategy(businessID: UUID, customerID?: UUID): Promise<GIACT> {
		...
		if (customerID) {
			const resolved = await resolveGiactStrategyModeForCustomer(customerID, businessID);
			strategyMode = resolved.strategyMode;
		}
	...
	public async fetch_giact_verification(
		...
		try {
			...
			const customerOrgIsSandboxForCase = await getCustomerOrgSandboxFlagForCaseBusinessDetails(customerID);

			const businessDetail = await getBusinessDetails(businessID, undefined, 2, customerOrgIsSandboxForCase);
	...
	private buildGiactRequest(bankAccount, giactServiceFlags, entityData, businessTin?: string): ServiceRequest {
		const { routingNumber, accountNumber, subtypeForAccountType } = applyGoldenTinSandboxBankOutboundOverrides(
			this.strategyMode,
			bankAccount,
			businessTin
		);
		return {
			...
			BankAccountEntity: {
				...
				AccountType: this.determineAccountType(subtypeForAccountType)
			},
			...entityData
		};
	}
	...
	private async attemptVerification(
		...
		const requestData = this.buildGiactRequest(bankAccount, giactServiceFlags, entityData, businessTin);
	...
	*/
	private prepareEntities(businessDetail, controlOwner, omitPhoneAndAddressFlag) {
		...
		// Build business entity (outbound FEIN may remap for SANDBOX + golden TIN; domain tin on business stays unchanged)
		const businessEntity: BusinessEntity = {
			BusinessName: businessDetail?.data?.name,
			PhoneNumber: businessPhone,
			FEIN: remapBusinessFeinForSandboxGiact(this.strategyMode, businessDetail?.data?.tin)
		};
		...
		const caseTin = businessDetail?.data?.tin;
		const businessEntityForGiact = applyGoldenTinSandboxBusinessOutboundOverrides(
			this.strategyMode,
			businessEntity,
			caseTin
		);
		...
		return {
			businessEntity: businessEntityForGiact
		};
	}
