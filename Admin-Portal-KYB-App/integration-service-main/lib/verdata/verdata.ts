...
import { EVENTS, INTEGRATION_ENABLE_STATUS, INTEGRATION_ID, TASK_STATUS } from "#constants";
import { getBusinessDetails, logger, sqlQuery, TIN_BEHAVIOR } from "#helpers/index";
...
import { isCompleteVerdataOrder } from "./typeguards";
...
export class Verdata extends TaskManager {
	...
	private async fetchBusinessDetailsAndCreateOrder(businessID: UUID) {
		...
		if (getBusinessDetailsResponse.status !== "success") {
			logger.error(
				...
				"[Verdata.createOrderFromBusinessDetails] Failed to get business details"
			);
		...
		const order: Omit<VerdataType.WorthInternalOrder, "business_id" | "task_id" | "phone"> = {
			name: data.name,
	...
	public async fetchPublicRecords(
		orderOverrideOrEmpty: Partial<VerdataType.WorthInternalOrder> | null = null,
		...
		const isBJLEnabled = taskId ? await this.isBJLEnabled(taskId) : false;

		let order: Partial<VerdataType.WorthInternalOrder>;
		if (isEmpty(orderOverrideOrEmpty)) {
			logger.info("fetchPublicRecords provided order is empty; fetching business details and creating order");

			if (!businessID) {
				const message = "fetchPublicRecords businessID is required when provided order is empty";
				logger.error(message);
				throw new Error(message);
			}

			order = await this.fetchBusinessDetailsAndCreateOrder(businessID as UUID);
			logger.info({ order }, "fetchPublicRecords fetched business details and created order");
		} else {
			order = { ...orderOverrideOrEmpty } as VerdataType.WorthInternalOrder;
			logger.info(
				{ order: orderOverrideOrEmpty },
				"fetchPublicRecords provided order is not empty; using provided order"
			);
		}
		order.task_id = taskId;
		order.business_id = order.business_id || (businessID as UUID);

		logger.info({ business_id: businessID, order }, "fetchPublicRecords order");
		const response = await this.getPublicRecords(order as VerdataType.WorthInternalOrder, { isBJLEnabled });
	...
	public async processFetchPublicRecordsTask(taskId: UUID) {
		...
		const { metadata } = await Verdata.getEnrichedTask(taskId);
		const order = isCompleteVerdataOrder(metadata) ? metadata : null;
		const response = await this.fetchPublicRecords(order, connection.business_id, taskId);
		if ("request_id" in response && response.request_id) {
			const record = response as VerdataType.Record;
			...
			if (!isBJLEnabled) {
				if (response.seller_id) {
					await VerdataUtil.upsertRecord(taskId, record);
			...
			await VerdataUtil.saveRawResponseToDB(record, connection.business_id, task);
		}
		return true;
	}
