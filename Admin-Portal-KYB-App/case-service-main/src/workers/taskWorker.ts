...
export const webhookQueue = new BullQueue(QUEUES.WEBHOOK);
...
export const initTaskWorker = () => {
	...
	webhookQueue.queue.process(QUEUE_EVENTS.SEND_WEBHOOK, 5, async (job: Job, done) => {
		await processJob(job, businessEventsHandler.gatherDataAndSendWebhookData.bind(businessEventsHandler));
		done();
	});
};
