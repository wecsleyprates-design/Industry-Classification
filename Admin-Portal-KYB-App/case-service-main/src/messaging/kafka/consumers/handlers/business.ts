...
import { taskQueue, webhookQueue } from "#workers";
		...
		try {
			...
			switch (event) {
				...
				case kafkaEvents.SEND_WEBHOOK:
					...
					if (!payload.business_id && payload.options?.business_id) {
						payload.business_id = payload.options.business_id;
					}
					await kafkaToQueue(webhookQueue, QUEUE_EVENTS.SEND_WEBHOOK, payload);
					break;
				...
				try {
					switch (event) {
						case WEBHOOK_EVENTS.BUSINESS_UPDATED: {
							...
							const businessId = body.options.business_id;
							...
							const [
								...
								kybDetails,
								processingStatementData
							] = await Promise.all([
								businesses.getCustomersByBusinessId(businessId),
								businesses.getBusinessByID(
									{ businessID: businessId, tinBehavior: TIN_BEHAVIOR.PLAIN },
									{ fetch_owner_details: true }
								),
								businesses.getCustomFieldsByBusinessId(businessId),
								getVerificationUploadsForBusiness(businessId),
								fetchDepositAccountInfo(businessId),
								getBusinessApplicantsForWebhooks(businessId),
								getEntityVerificationDetails(businessId),
								getBusinessKybDetails(businessId).catch((error: any) => {
									logger.error(
										error,
										`gatherDataAndSendWebhookData: Something went wrong while fetching KYB details for ${businessId} ${error.message}`
									);
									return null;
								}),
								getBusinessProcessingHistory(businessId as UUID).catch((e: any) => {
									logger.error(
										e,
										`gatherDataAndSendWebhookData: Something went wrong while fetching processing statements data for ${businessId}: ${e.message}`
									);
									return null;
								})
							]);
							...
							logger.info(customFields, `gatherDataAndSendWebhookData: Custom Fields for ${businessId}`);
							logger.info(
								businessVerificationUploads,
								`gatherDataAndSendWebhookData: businessVerificationUploads for ${businessId}`
							);
							...
							logger.debug(
								depositAccountResult,
								`gatherDataAndSendWebhookData: depositAccountResult for ${businessId}`
							);
							...
							if (processingStatementData) {
								businessDetails["processing_statements"] = processingStatementData.map(record => ({
							...
							for (const customer of customersData) {
								...
								logger.info(
									...
									`gatherDataAndSendWebhookData: Webhook payload for business ${businessId}, customer ${customer.customer_id}`
								);
