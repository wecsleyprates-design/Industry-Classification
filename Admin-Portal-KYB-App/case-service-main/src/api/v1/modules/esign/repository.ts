...
class EsignRepository {
	async addTemplate(
		userId: UUID,
		opts?: Partial<Pick<IDataDocumentTemplate, "customer_id" | "template_id" | "name" | "template_fields" | "metadata" | "base_name">>
	...
	async getCustomerTemplates(
		customerId: UUID
	): Promise<(IRelCustomerTemplates & { name: string; created_at: Date; version: number; tags: string[]; base_name: string | null })[]> {
		const result = await db<
			Array<IRelCustomerTemplates & { name: string; created_at: Date; version: number; tags: string[]; base_name: string | null }>
		>("esign.rel_customer_templates")
			.select(
				"esign.rel_customer_templates.*",
				"esign.data_document_templates.name",
				"esign.data_document_templates.base_name",
