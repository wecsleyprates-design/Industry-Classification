...
// dynamic owner{n}_{ownerStringFields} pattern where n is 1-16
const ownerIndexPattern = "([1-9]|1[0-6])";
const ownerStringPattern = new RegExp(`^owner${ownerIndexPattern}_(${ownerStringFields})$`, "u"); // strings
const ownerEmailPattern = new RegExp(`^owner${ownerIndexPattern}_email$`, "u"); // string.email
const ownerOwnershipPattern = new RegExp(`^owner${ownerIndexPattern}_ownership_percentage$`, "u"); // number
const ownerUUIDPattern = new RegExp(`^owner${ownerIndexPattern}_id$`, "u"); // UUID (optional)
...
export const schema = {
	getBusinessByID: {
		...
		query: Joi.object({
			...
			customer_org_sandbox: Joi.string().valid("true", "false").optional()
		})
	},
	...
	/**
	 * Internal sync endpoint (integration-service) for updating business display profile:
	 * - legal/display `name` (data_businesses.name)
	 * - `dba_names` rows (data_business_names, non-primary managed via updateDBAName)
	 *
	 * This route is intentionally narrow: it exists only to keep case overview/listings in sync with KYB fact overrides.
	 */
	updateBusinessDisplayProfile: {
		params: Joi.object({
			businessID: requiredUUID
		}),
		body: Joi.object({
			...
			const hasDBA = value.dba_names !== undefined;
			if (!hasName && !hasDBA) {
				return helpers.error("any.custom", { message: "name and/or dba_names must be provided" });
			}
			return value;
		})
	}
