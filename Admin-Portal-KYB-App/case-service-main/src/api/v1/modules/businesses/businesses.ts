...
import {
	...
	TIN_BEHAVIOR,
	CORE_PERMISSIONS,
	PROCESSING_HISTORY_SECTIONS_COLUMN_MAPPING,
	PROCESSING_HISTORY_FIELDS_COLUMN_MAPPING
...
class Businesses {
	...
	async getBusinessByID(
		...
		userInfo?: UserInfo,
		options?: { isInternalServiceRequest?: boolean; customerOrgIsSandbox?: boolean }
	): Promise<
	...
	> {
		const hasPermission: boolean = await hasDataPermission(userInfo, CORE_PERMISSIONS.READ_PII_DATA);
		// Specifically for Sandbox GIACT verification with use of Golden TIN, needs to provide FEIN to integration service
		const hasPermissionForGiactSandbox =
			options?.isInternalServiceRequest !== false && options?.customerOrgIsSandbox !== false;
		...
		if (!hasPermission || !hasPermissionForGiactSandbox) {
			response.tin = null;
		} else {
	...
	async getProgression(params, query, userInfo: UserInfo, { authorization }) {
		try {
			...
			// Find the fields required for the "processing_history" stage, then map those field names to their corresponding database column names, and finally build a list of all the required data fields
			const processingHistoryStageConfig = progressionConfig?.find(row => row.stage.toLowerCase() == "processing_history")?.config
				?.fields;

			const requiredProcessingHistoryDetails = (processingHistoryStageConfig ?? [])
				.filter(field => field.status === "Required" || field.status === "Always Required")
				.filter(field => Boolean(PROCESSING_HISTORY_SECTIONS_COLUMN_MAPPING[field.section_name]))
				.filter(field => Boolean(PROCESSING_HISTORY_FIELDS_COLUMN_MAPPING[field.name]))
				.reduce<Record<string, string[]>>((acc, field) => {
					const sectionKey = PROCESSING_HISTORY_SECTIONS_COLUMN_MAPPING[field.section_name];
					const column = PROCESSING_HISTORY_FIELDS_COLUMN_MAPPING[field.name];
					if (!acc[sectionKey]) { 
						acc[sectionKey] = [];
					}
					acc[sectionKey].push(column);
					return acc;
				}, {});
			...
			for (const item of progressionConfig) {
				...
				switch (item.stage.split(":")[0]) {
					...
					case "ownership": {
						...
						// If IDV is enabled, stage is complete if all owners are verified.
						// Bypass this if submit with unverified identity is enabled or if the user is a guest owner.
						if (idvEnabled && business.owner_details_exists) {
							isStageCompleted = !!(allOwnersVerified || submitWithUnverifiedIdentityEnabled || isGuestOwner);
						}
						// If IDV is disabled, stage is complete when owner details are provided.
						if (!idvEnabled && business.owner_details_exists) {
							isStageCompleted = true;
						}
						// If extended ownership is disabled, stage is complete only if a control owner is present.
					...
					case "processing_history": {
						if (Array.isArray(processingHistory) && processingHistory.length) {
							isStageCompleted = true;
							...
							for (const record of processingHistory) {
								const dataSections = [
									...
									["general_data", record.general_data],
									["seasonal_data", record.seasonal_data]
								];

								// Iterate through each section and check if all required fields are completed
								for (const [sectionKey, sectionData] of dataSections) {
									const requiredSectionDetails = requiredProcessingHistoryDetails[sectionKey];

									if (!requiredSectionDetails?.length) continue;

									const isSectionComplete = 
										sectionData &&
										requiredSectionDetails?.every((key) => {
						...
						if (!customFieldsProcessedDetails.fields.length) {
							...
							if (hasRequiredFields) {
								...
								isStageCompleted = customFieldsProcessedDetails.fields.every(field => {
									...
									const isVisible = !fieldVisibilityMap[field.internalName]?.invisible;
					...
					case "review": {
						...
						if (isEsignEnabled) {
							...
							const preFilledArray = selectedTemplates.map(template => {
								...
								let eSignTemplateId = template.base_name?.concat(`_v${template.version}`);
								
								// Legacy fallback
								if (!template.base_name) {
	...
	/**
	 * Internal integration sync endpoint:
	 * - updates data_businesses.name (when provided)
	 * - updates data_business_names non-primary DBA rows (when dba_names is provided, including empty array to clear)
	 *
	 * Non-primary DBA invariants are handled by `updateDBAName`.
	 */
	async updateBusinessDisplayProfile(
		body: {
			name?: string | null;
			dba_names?: { name: string; is_primary?: boolean }[] | null;
		},
		...
		const businessRecord = await db("data_businesses")
			.select("id", "name")
		...
		let effectiveBusinessName: string = businessRecord.name;
		...
		if (nameToSet) {
			effectiveBusinessName = nameToSet;
		...
		// Only touch DBAs when the key exists as an array at the API boundary.
		if (Array.isArray(body?.dba_names)) {
			const dbaNames = (body.dba_names ?? [])
				.map(dba => ({ name: dba.name.trim() }))
				.filter(dba => dba.name !== "");

			await this.updateDBAName(
				{ name: effectiveBusinessName },
				dbaNames,
		...
		// Name-only update: update the primary business name row without touching non-primary DBAs.
		if (nameToSet) {
			const primaryRow = await db("data_business_names")
				.select("id")
			...
			if (primaryRow) {
				...
				await db("data_business_names").insert({
					business_id: businessID,
					name: effectiveBusinessName,
					is_primary: true,
					created_by: userID,
			...
			await sendEventToGatherWebhookData(
				[WEBHOOK_EVENTS.BUSINESS_UPDATED],
				{ business_id: businessID }
			);
		}
