...
export async function validateOwnerFields(mapper: Mapper): Promise<void> {
	...
	const isExtendedOwnershipEnabled: boolean = !!mapper.getAdditionalMetadata()?.extendedOwnership;
	// Default to 5. 1 Control Owner + up to 4 Beneficial Owners
	const MAX_OWNERS: number = mapper.getAdditionalMetadata()?.extendedOwnership?.maxTotalOwners ?? 5;
	...
	ownerNumbers.forEach(id => {
		...
		addOwnerBody.push({
			owner_type: ownerType?.value as OwnerType,
		});
		...
		if (
			mapper instanceof BulkCreateBusinessMap && 
			beneficialOwners == ownerNumbers.length &&
			!isExtendedOwnershipEnabled
		) {
	...
	if (isExtendedOwnershipEnabled) handleBulkExtendedOwnershipValidation(mapper, addOwnerBody, []);
...
export function getOwnerFields(mode: "create" | "update", options?: {extendedOwnership?: Business.ExtendedOwnerLimits}) {
	const MAX_OWNERS: number = options?.extendedOwnership?.maxTotalOwners ?? 5;
...
export async function updateValidateOwnerFields(mapper: Mapper) {
	...
	if (isExtendedOwnershipEnabled) handleBulkExtendedOwnershipValidation(mapper, newOwners, currentOwners);
...
// Runs extended ownership validation and, in the event of a failure, mutates the thrown error into a mapper error
// and returns all of the customer's extended ownership configurations in the error message.
function handleBulkExtendedOwnershipValidation(
	mapper: Mapper,
	modifiedOwners: Array<Business.Owner | { id?: string | undefined }>, 
	existingOwners?: Array<Business.Owner>
) {
	const ownerLimits: Business.ExtendedOwnerLimits = mapper.getAdditionalMetadata()?.extendedOwnership;
		try {
			Owners.validateExtendedOwnership(
				modifiedOwners, 
				existingOwners ?? [], // Existing owners would not exist when creating a new business.
				ownerLimits
			);
		} catch (_error) {
			let errorMessage: string = "Invalid owner list.";
			if (ownerLimits?.maxTotalOwners != null) errorMessage += ` Total owners may not exceed ${ownerLimits?.maxTotalOwners}.`;
			if (ownerLimits?.maxControlOwners != null) errorMessage += ` Total ${OWNER_TYPES.CONTROL} owners may not exceed ${ownerLimits?.maxControlOwners}.`;
			if (ownerLimits?.maxBeneficialOwners != null) errorMessage += ` Total ${OWNER_TYPES.BENEFICIARY} owners may not exceed ${ownerLimits?.maxBeneficialOwners}.`;

			logger.error({ error: _error }, "Extended ownership validation failed");
			throw new MapperError(errorMessage);
		}
}