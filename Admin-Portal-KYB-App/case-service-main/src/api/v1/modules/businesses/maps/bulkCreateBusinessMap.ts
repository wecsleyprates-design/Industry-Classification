...
// Construct "owner{n}_" fields for n = 2, 3, 4, 5, etc.

export class BulkCreateBusinessMap extends Mapper {
	static buildMap(options?: {}): MapperDefinition {
		return {
			...
			tables: {
				...
				data_owners: {
					order: 3,
					fields: getOwnerFields("create", options),
	...
	constructor(input: Map<string, any>, runId = uuid(), options?: {}, threshold = 0.2) {
		super(BulkCreateBusinessMap.buildMap(options), input, { runId: runId as UUID, threshold, knexInstance: db });
	}
