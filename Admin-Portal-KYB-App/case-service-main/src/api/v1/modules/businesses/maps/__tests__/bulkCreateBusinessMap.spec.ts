...
import { getApplicantConfigFields, getOwnerFields } from "../fields";
...
describe("BulkCreateBusinessMap extended ownership options", () => {
	let tracker: Tracker;
	...
	it("getOwnerFields returns 16 owner sets when extendedOwnership.maxTotalOwners is 16", () => {
		...
		const ownerFields = getOwnerFields("create", options);
	...
	it("allows multiple control owners when extendedOwnership is enabled", async () => {
		...
		const request = {
			external_id: "multi-control-test",
			tin: "111222333",
			name: "Multi Control Corp",
		...
		const mapper = new BulkCreateBusinessMap(new Map(Object.entries(request)), "run-1", options);
		mapper.setAdditionalMetadata({
			customerID: "cust-1",
			extendedOwnership: options.extendedOwnership
		});
		(businessLookupHelper as jest.Mock).mockRejectedValue(new Error("Business not found"));
		const match = await mapper.match();
	...
	it("fails when control owners exceeds maxControlOwners", async () => {
		const options = {
			extendedOwnership: {
				maxTotalOwners: 5,
				maxControlOwners: 1,
