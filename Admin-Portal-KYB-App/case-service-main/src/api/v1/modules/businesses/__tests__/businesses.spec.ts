...
describe("Businesses", () => {
	...
	describe("getProgression", () => {
		...
		it("should return processing history stage as completed when all required details present", async () => {
			db.mockImplementation(() => ({
				select: jest.fn().mockReturnThis(),
				join: jest.fn().mockReturnThis(),
				where: jest.fn().mockReturnThis(),
			...
			sqlTransaction.mockResolvedValueOnce([
				...
				{
					rowCount: 1,
					rows: [{ id: "caseID", status_code: CASE_STATUS.ONBOARDING, customer_id: "customerID" }]
				},
				{ rowCount: 0, rows: [] },
				{ rowCount: 0, rows: [] },
				{ rowCount: 0, rows: [] }
			...
			getBusinessProcessingHistory.mockResolvedValueOnce([
				{
					card_data: {
			...
			const processingHistoryStage = result.completed_stages?.find(stage => stage.stage === "processing_history");
			expect(processingHistoryStage?.is_stage_truly_completed).toBe(true);
		});

		it("should return processing history stage as incomplete when required details are missing", async () => {
			db.mockImplementation(() => ({
				select: jest.fn().mockReturnThis(),
				join: jest.fn().mockReturnThis(),
				where: jest.fn().mockReturnThis(),
			...
			const processingHistoryStage = result.completed_stages?.find(stage => stage.stage === "processing_history");
			expect(processingHistoryStage?.is_stage_truly_completed).toBeUndefined();
		});

		it("should return processing history stage as incomplete when required section is missing", async () => {
			db.mockImplementation(() => ({
				select: jest.fn().mockReturnThis(),
				join: jest.fn().mockReturnThis(),
				where: jest.fn().mockReturnThis(),
