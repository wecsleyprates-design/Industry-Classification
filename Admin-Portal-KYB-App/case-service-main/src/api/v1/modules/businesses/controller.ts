...
import { getBusinessMapOptions, parseBulkProcessBody, normalizeBulkRows, initMapper } from "./helpers/bulkProcessBusiness";
...
export const controller = {
	...
	getBusinessByID: catchAsync(async (req, res) => {
		const isInternalServiceRequest =
			typeof req.originalUrl === "string" && req.originalUrl.includes("/internal/businesses/");
		const customerOrgIsSandbox =
		req.query?.customer_org_sandbox === "true" ? true : req.query?.customer_org_sandbox === "false" ? false : undefined;
		const response = await businesses.getBusinessByID(req.params, req.query, res.locals.user, {
			isInternalServiceRequest,
			customerOrgIsSandbox
		});
	...
	bulkValidateBusinesses: catchAsync(async (req, res) => {
		...
		// Fetches business map options per customer configuration
		const businessMapOptions: BulkBusinessMapOptions = await getBusinessMapOptions(customerID);
		...
		for (const key in body) {
			...
			const mapper = initMapper(input, runMode, undefined, businessMapOptions);
			...
			mapper.setAdditionalMetadata({
				...
				extendedOwnership: businessMapOptions?.extendedOwnership
			});
	...
	bulkProcessBusiness: catchAsync(async (req, res) => {
		...
		for (const index in body) {
			...
			try {
				...
				const mapper = initMapper(input, runMode, runId, businessMapOptions);
				mapper.setAuth(req.headers.authorization);
				mapper.setAdditionalMetadata({
					userID: res.locals.user?.user_id ?? envConfig.ENTERPRISE_APPLICANT_ID,
					customerID: customerID,
				...
				try {
					...
					await sendEventToFetchAdverseMedia(businessID, customerID, metadata?.data_cases?.id);
	...
	bulkProcessBusinessFields: catchAsync(async (req, res) => {
		const input = new Map();
		const runId = uuid();
		const { customerID } = req.params;
		...
		const mapper = initMapper(input, "create", runId, businessMapOptions);
	...
	updateBusinessDisplayProfile: catchAsync(async (req, res) => {
		const { businessID } = req.params;
		const { userId, name, dba_names } = req.body;

		const response = await businesses.updateBusinessDisplayProfile(
			{ name, dba_names },
			{ businessID },
			{ user_id: userId }
		);
		res.jsend.success(response, "Business display profile updated successfully");
	})
};
