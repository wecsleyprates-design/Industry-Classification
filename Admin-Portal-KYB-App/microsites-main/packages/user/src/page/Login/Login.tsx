...
import { getAllPermissions } from "@/lib/helper";
	...
	useEffect(() => {
		if (loginData) {
			if (
				...
				const mappedPermissions = getAllPermissions(
				...
				setItem("permissions", mappedPermissions ?? "");
				setItem(
					LOCALSTORAGE.allPermissions,
					loginData.data.permissions.map((item) => item.code),
				);
