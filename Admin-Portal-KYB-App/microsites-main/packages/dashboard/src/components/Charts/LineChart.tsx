					...
					label: (context: any) => {
						return chartOptions?.y.format === "number"
							...
							: formatPriceWithSuffix(
									Number(context.formattedValue.replaceAll(",", "")),
								);
					},
				},
			},
			...
			y: {
				...
				ticks: {
					...
					callback: function (value: number) {
						return chartOptions?.y.format === "number"
							? value.toFixed(1)
							: formatPriceWithSuffix(value); // to add %
					},
				},
			},
		},
					...
					{isLoading ? (
						...
						<div className={chartOptions?.componentClass ?? "h-full"}>
							<div className="w-full  flex justify-center">
								<div
									className={`w-full ${
										chartOptions?.chartClass ?? "h-[300px] 2xl:h-[400px] 	"
									}`}
								>
									<Line height={200} options={options} data={chartData} />
								</div>
							</div>
							<div className="w-full  flex justify-center mt-8 gap-x-6">
