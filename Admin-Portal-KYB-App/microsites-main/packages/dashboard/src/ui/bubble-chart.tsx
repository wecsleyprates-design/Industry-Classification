...
import {
	type IndustryExposureDataObject,
...
export const BubbleChart: FC<Props> = ({ title, chartData }) => {
						...
						options={{
									...
									callbacks: {
										title: (context: any) => {
											if (bubbleChartTooltipData) {
												return bubbleChartTooltipData[context[0]?.datasetIndex];
											} else return "";
										},
										label: () => {
											return "";
							...
							scales: {
								...
								x: {
									...
									ticks: {
										stepSize:
											maxValues.count < 5 ? 1 : Math.ceil(maxValues.count / 5),
										font: {
											size: 14,
										},
									},
