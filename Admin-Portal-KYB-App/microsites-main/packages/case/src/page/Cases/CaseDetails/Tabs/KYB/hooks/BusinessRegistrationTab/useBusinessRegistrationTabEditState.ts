...
/**
 * Hook that manages edit state for BusinessRegistrationTab.
 * Handles save status tracking, override timestamps, and edit completion.
 */
export function useBusinessRegistrationTabEditState(
	caseId: string,
	businessId: string,
	...
	const latestOverrideTimestamp = useMemo(() => {
		const allFactsData = [data.getFactsKybData?.data].filter(Boolean);
