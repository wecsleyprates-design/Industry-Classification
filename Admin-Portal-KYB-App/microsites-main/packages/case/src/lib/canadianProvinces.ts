...
export function getCanadianProvince(address: string): string | null {
	...
	const matchedProvince = canadianProvinces.find((province) =>
		normalizedAddress.includes(province.toUpperCase()),
	);
...
export function extractCanadianAddressComponents(address: string): {
	province: string;
	postalCode: string;
	country: string;
} {
	...
	// Normalize postal code to remove internal space (e.g. "T2Y 2T5" → "T2Y2T5")
	// so that addresses from different sources produce identical output.
...
export function getProvinceFromJurisdictionCode(
