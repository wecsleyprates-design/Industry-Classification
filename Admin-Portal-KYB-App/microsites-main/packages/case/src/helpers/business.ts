 ...
 * 3. If no addresses are available, return null
 *
...
export const getPrimaryBusinessAddress = (
	...
	if (!addressSources?.length) return null;
...
export const extractGeocodingDetails = (
	...
	if (response.status !== "OK" || response.results.length === 0) {
		const errorMessage = response.error_message
			? `Geocoding API error: ${response.error_message}`
			: "Geocoding API error or no results found.";
