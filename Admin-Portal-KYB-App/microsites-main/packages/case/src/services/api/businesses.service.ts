...
export const getCustomerBusinessCases = async (
	payload: IGetBusinessCases,
): Promise<GetCasesResponse> => {
	const { businessId, customerID, params } = payload;
	const url = `${MICROSERVICE.CASE}/customers/${customerID}/businesses/${businessId}/cases`;
...
export const getBusinessCases = async (
	...
	const url = `${MICROSERVICE.CASE}/businesses/${businessId}/cases`;
	const { data } = await api.get(url, {
		params,
	});
	return data;
...
export const createBusinesses = async (payload: NewBusinessPayload) => {
	...
	const url: string = `${MICROSERVICE.CASE}/businesses/customers/${customerId}/bulk/process`;
...
export const getRelatedBusinesses = async (
	...
	businessId: string,
	params?: string,
): Promise<PaginatedAPIResponse<RelatedBusiness>> => {
	const { data } = await api.get(
		`${
			MICROSERVICE.CASE
