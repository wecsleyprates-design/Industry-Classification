...
def render_field(f):
    ...
    with st.expander(f"🔍 Full lineage — {f['label']}", expanded=False):
        ...
        with c2:
            ...
            bid_example = "'fed4f94f-4bc3-47ef-b01e-f870c857e1bc'  -- replace with your business UUID"
            ...
            if f.get("sql") and not f["sql"].startswith("--"):
                field_sql = (f["sql"]
                    .replace("'<id>'", bid_example)
                    .replace("'<business_id>'", bid_example)
                )
            ...
            if is_redshift:
                st.code(
                    ...
                    "    sslmode='require'\n"
                    ")\n\n"
                    "BUSINESS_ID = 'fed4f94f-4bc3-47ef-b01e-f870c857e1bc'  # replace\n\n"
                    "df = pd.read_sql(\n"
    ...
    if bg_section == "🏢 Business Details Card":
        ...
        BDET = [
            ...
            {"label":"Mailing Address","fact":"mailing_address","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='mailing_address';"},
            {"label":"Business Age (Formation Date)","fact":"formation_date","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT name, json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('formation_date','year_established');"},
            ...
            {"label":"Avg. Annual Revenue","fact":"revenue_equally_weighted_average","api":"/facts/business/:id/financials","editable":False,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='revenue_equally_weighted_average';"},
            {"label":"Net Income","fact":"net_income","api":"/facts/business/:id/financials","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('net_income','is_net_income');"},
            {"label":"Corporation Type","fact":"corporation","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='corporation';"},
            ...
            {"label":"Business Phone Number","fact":"business_phone","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='business_phone';"},
            {"label":"Business Email","fact":"email","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='email';"},
            {"label":"Minority Business Enterprise","fact":"minority_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT name, json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('minority_owned','woman_owned','veteran_owned');"},
            {"label":"Woman-Owned Business","fact":"woman_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='woman_owned';"},
            {"label":"Veteran-Owned Business","fact":"veteran_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='veteran_owned';"},
        ]
        for f in BDET:
            render_field(f)
...
no evidence. '-' should not appear unless an integration task failed or is still running.
        ...
        INDUSTRY = [
            ...
            {"label":"NAICS Description","fact":"naics_description","api":"/facts/business/:id/details","editable":False,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='naics_description';"},
            {"label":"MCC Code","fact":"mcc_code / mcc_code_found / mcc_code_from_naics","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT name, json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>'\nAND name IN ('mcc_code','mcc_code_found','mcc_code_from_naics','mcc_description');"},
            {"label":"MCC Description","fact":"mcc_description","api":"/facts/business/:id/details","editable":False,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') AS mcc_desc FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='mcc_description';\n-- If shows 'Fallback MCC per instructions...' → AI enrichment had no vendor evidence"},
        ]
        for f in INDUSTRY:
            render_field(f)
    ...
    if biz_card == "🏷️ Business Registration Card":
        ...
        BIZ_REG = [
            {"label":"Business Name (in Business Registration card)","fact":"legal_name","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT json_extract_path_text(value::json, 'value') AS legal_name FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='legal_name';"},
        ...
        st.code("SELECT json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='shareholder_document';", language="sql")
    ...
    if ci_card == "🗺️ Addresses Card":
        ...
        ADDR_FIELDS = [
            ...
            {"label":"Reported Addresses + Verified / Deliverable badges","fact":"addresses + addresses_deliverable + address_verification","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT jsonb_array_length(value->'value') AS count, json_extract_path_text(value::json, 'value') AS addrs\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='addresses';\n\nSELECT json_extract_path_text(value::json, 'value') AS deliverable_addrs FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='addresses_deliverable';"},
        ]
        ...
        BN_FIELDS = [
            ...
            {"label":"Reported Names (from public records)","fact":"names_found","api":"/facts/business/:id/kyb","editable":False,
             ...
             "storage":["rds_warehouse_public.facts name='names_found' (array)"],"null_cause":"Empty if no vendor found any names.","badges":[],"react_src":("ADMIN_PORTAL","microsites-main/packages/case/src/page/Cases/CaseDetails/Tabs/KYB/components/BusinessNamesCard.tsx",1,30,"reported names section"),"sql":"SELECT json_extract_path_text(value::json, 'value') AS names_found FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='names_found';"},
        ]
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    WEB_FIELDS = [
        {"label":"Website URL","fact":"website","api":"/facts/business/:id/details","editable":True,
         ...
         "sql":"SELECT name, json_extract_path_text(value::json, 'value') FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('website','website_found');"},
...
SELECT
    ...
    received_at
FROM rds_warehouse_public.facts
WHERE business_id = 'fed4f94f-4bc3-47ef-b01e-f870c857e1bc'  -- replace
