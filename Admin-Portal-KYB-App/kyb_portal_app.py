    ...
    with st.expander(f"🔍 Full lineage — {f['label']}", expanded=False):
        ...
        with c2:
            ...
            if not sql_for_python or sql_for_python.startswith("--"):
                sql_for_python = (
                    "SELECT name, (value::jsonb)->>'value' AS val, "
                    "(value::jsonb)->'source'->>'platformId' AS winning_pid, "
                    "(value::jsonb)->'source'->>'confidence' AS confidence "
    ...
    if bg_section == "🏢 Business Details Card":
        ...
        BDET = [
            {"label":"Provided Business Name","fact":"business_name","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' AS name, (value::jsonb)->'source'->>'platformId' AS pid,\n       (value::jsonb)->'override'->>'value' AS override\nFROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name='business_name';",
            ...
            {"label":"Legal Business Name","fact":"legal_name","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT (value::jsonb)->>'value' AS legal_name FROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name='legal_name';"},
            {"label":"DBA (Doing Business As)","fact":"dba / dba_found","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT name, (value::jsonb)->>'value' FROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name IN ('dba','dba_found');"},
            ...
            {"label":"Mailing Address","fact":"mailing_address","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='mailing_address';"},
            {"label":"Business Age (Formation Date)","fact":"formation_date","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT name, (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('formation_date','year_established');"},
            {"label":"Annual Revenue","fact":"revenue","api":"/facts/business/:id/financials","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' AS revenue, (value::jsonb)->'source'->>'platformId' AS pid\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='revenue';"},
            {"label":"Avg. Annual Revenue","fact":"revenue_equally_weighted_average","api":"/facts/business/:id/financials","editable":False,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='revenue_equally_weighted_average';"},
            {"label":"Net Income","fact":"net_income","api":"/facts/business/:id/financials","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('net_income','is_net_income');"},
            {"label":"Corporation Type","fact":"corporation","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='corporation';"},
            {"label":"Number of Employees","fact":"num_employees","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' AS employees, (value::jsonb)->'source'->>'platformId' AS pid\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='num_employees';"},
            {"label":"Business Phone Number","fact":"business_phone","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='business_phone';"},
            {"label":"Business Email","fact":"email","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='email';"},
            {"label":"Minority Business Enterprise","fact":"minority_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT name, (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('minority_owned','woman_owned','veteran_owned');"},
            {"label":"Woman-Owned Business","fact":"woman_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='woman_owned';"},
            {"label":"Veteran-Owned Business","fact":"veteran_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='veteran_owned';"},
        ]
        for f in BDET:
            render_field(f)
...
no evidence. '-' should not appear unless an integration task failed or is still running.
        ...
        INDUSTRY = [
            ...
            {"label":"NAICS Description","fact":"naics_description","api":"/facts/business/:id/details","editable":False,
             ...
             "sql":"SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='naics_description';"},
            {"label":"MCC Code","fact":"mcc_code / mcc_code_found / mcc_code_from_naics","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT name, (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>'\nAND name IN ('mcc_code','mcc_code_found','mcc_code_from_naics','mcc_description');"},
            {"label":"MCC Description","fact":"mcc_description","api":"/facts/business/:id/details","editable":False,
             ...
             "sql":"SELECT (value::jsonb)->>'value' AS mcc_desc FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='mcc_description';\n-- If shows 'Fallback MCC per instructions...' → AI enrichment had no vendor evidence"},
        ]
        for f in INDUSTRY:
            render_field(f)
    ...
    if biz_card == "🏷️ Business Registration Card":
        ...
        BIZ_REG = [
            {"label":"Business Name (in Business Registration card)","fact":"legal_name","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::jsonb)->>'value' AS legal_name FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='legal_name';"},
        ...
        st.code("SELECT (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='shareholder_document';", language="sql")
    ...
    if ci_card == "🗺️ Addresses Card":
        ...
        ADDR_FIELDS = [
            ...
            {"label":"Reported Addresses + Verified / Deliverable badges","fact":"addresses + addresses_deliverable + address_verification","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT jsonb_array_length(value->'value') AS count, (value::jsonb)->>'value' AS addrs\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='addresses';\n\nSELECT (value::jsonb)->>'value' AS deliverable_addrs FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='addresses_deliverable';"},
        ]
        ...
        BN_FIELDS = [
            ...
            {"label":"Reported Names (from public records)","fact":"names_found","api":"/facts/business/:id/kyb","editable":False,
             ...
             "storage":["rds_warehouse_public.facts name='names_found' (array)"],"null_cause":"Empty if no vendor found any names.","badges":[],"react_src":("ADMIN_PORTAL","microsites-main/packages/case/src/page/Cases/CaseDetails/Tabs/KYB/components/BusinessNamesCard.tsx",1,30,"reported names section"),"sql":"SELECT (value::jsonb)->>'value' AS names_found FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='names_found';"},
        ]
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    WEB_FIELDS = [
        {"label":"Website URL","fact":"website","api":"/facts/business/:id/details","editable":True,
         ...
         "sql":"SELECT name, (value::jsonb)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('website','website_found');"},
...
SELECT
    f.name,
    f.(value::jsonb)->>'value'                              AS winning_value,
    f.(value::jsonb)->'source'->>'platformId'               AS winning_platform_id,
    f.(value::jsonb)->'source'->>'confidence'               AS winning_confidence,
    f.(value::jsonb)->'override'->>'value'                  AS analyst_override,
    f.(value::jsonb)->'override'->>'userId'                 AS overridden_by,
    jsonb_array_length(COALESCE(f.(value::jsonb)->'alternatives','[]'::jsonb)) AS num_alternatives,
    f.received_at
...
SQL_FACTS = (
    "SELECT name, (value::jsonb)->>'value' AS value, "
    "(value::jsonb)->'source'->>'platformId' AS winning_pid, "
    "(value::jsonb)->'source'->>'confidence' AS confidence, "
    "(value::jsonb)->'override'->>'value' AS analyst_override "
    "FROM rds_warehouse_public.facts "
...
3. ALWAYS provide SQL to verify data when relevant:
   - Facts: SELECT name, (value::jsonb)->>'value', (value::jsonb)->'source'->>'platformId' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='<fact>';
