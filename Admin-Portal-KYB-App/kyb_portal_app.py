    ...
    with st.expander(f"🔍 Full lineage — {f['label']}", expanded=False):
        ...
        with c2:
            ...
            if not sql_for_python or sql_for_python.startswith("--"):
                sql_for_python = (
                    "SELECT name, (value::json)->>'value' AS val, "
                    "(value::json)->'source'->>'platformId' AS winning_pid, "
                    "(value::json)->'source'->>'confidence' AS confidence "
    ...
    if bg_section == "🏢 Business Details Card":
        ...
        BDET = [
            {"label":"Provided Business Name","fact":"business_name","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' AS name, (value::json)->'source'->>'platformId' AS pid,\n       (value::json)->'override'->>'value' AS override\nFROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name='business_name';",
             "python":"import psycopg2, pandas as pd\nconn = psycopg2.connect(host='<host>',dbname='<db>',user='<u>',password='<pw>')\ndf = pd.read_sql(\"SELECT value FROM rds_warehouse_public.facts WHERE business_id=%s AND name='business_name'\",conn,params=['<id>'])\nprint(df['value'].iloc[0])"},
            {"label":"Legal Business Name","fact":"legal_name","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT (value::json)->>'value' AS legal_name FROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name='legal_name';"},
            {"label":"DBA (Doing Business As)","fact":"dba / dba_found","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT name, (value::json)->>'value' FROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name IN ('dba','dba_found');"},
            {"label":"Business Address","fact":"primary_address","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT name, (value::json)->>'value' FROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name IN ('primary_address','primary_address_string','addresses');"},
            {"label":"Mailing Address","fact":"mailing_address","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='mailing_address';"},
            {"label":"Business Age (Formation Date)","fact":"formation_date","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT name, (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('formation_date','year_established');"},
            {"label":"Annual Revenue","fact":"revenue","api":"/facts/business/:id/financials","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' AS revenue, (value::json)->'source'->>'platformId' AS pid\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='revenue';"},
            {"label":"Avg. Annual Revenue","fact":"revenue_equally_weighted_average","api":"/facts/business/:id/financials","editable":False,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='revenue_equally_weighted_average';"},
            {"label":"Net Income","fact":"net_income","api":"/facts/business/:id/financials","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('net_income','is_net_income');"},
            {"label":"Corporation Type","fact":"corporation","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='corporation';"},
            {"label":"Number of Employees","fact":"num_employees","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' AS employees, (value::json)->'source'->>'platformId' AS pid\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='num_employees';"},
            {"label":"Business Phone Number","fact":"business_phone","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='business_phone';"},
            {"label":"Business Email","fact":"email","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='email';"},
            {"label":"Minority Business Enterprise","fact":"minority_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT name, (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('minority_owned','woman_owned','veteran_owned');"},
            {"label":"Woman-Owned Business","fact":"woman_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='woman_owned';"},
            {"label":"Veteran-Owned Business","fact":"veteran_owned","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='veteran_owned';"},
        ]
        for f in BDET:
            render_field(f)
...
no evidence. '-' should not appear unless an integration task failed or is still running.
        ...
        INDUSTRY = [
            ...
            {"label":"NAICS Description","fact":"naics_description","api":"/facts/business/:id/details","editable":False,
             ...
             "sql":"SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='naics_description';"},
            {"label":"MCC Code","fact":"mcc_code / mcc_code_found / mcc_code_from_naics","api":"/facts/business/:id/details","editable":True,
             ...
             "sql":"SELECT name, (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>'\nAND name IN ('mcc_code','mcc_code_found','mcc_code_from_naics','mcc_description');"},
            {"label":"MCC Description","fact":"mcc_description","api":"/facts/business/:id/details","editable":False,
             ...
             "sql":"SELECT (value::json)->>'value' AS mcc_desc FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='mcc_description';\n-- If shows 'Fallback MCC per instructions...' → AI enrichment had no vendor evidence"},
        ]
        for f in INDUSTRY:
            render_field(f)
    ...
    if biz_card == "🏷️ Business Registration Card":
        ...
        BIZ_REG = [
            {"label":"Business Name (in Business Registration card)","fact":"legal_name","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT (value::json)->>'value' AS legal_name FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='legal_name';"},
            {"label":"Tax ID Number (EIN) + ✅/⚠️ Verified badge","fact":"tin + tin_match_boolean","api":"/facts/business/:id/kyb","editable":True,
             ...
             "sql":"SELECT name, (value::json)->>'value' AS val FROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name IN ('tin','tin_submitted','tin_match','tin_match_boolean');",
        ...
        SOS_FIELDS = [
            {"label":"SoS Verified / Missing Active Filing badge","fact":"sos_filings[n].active","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT name, (value::json)->>'value' AS val FROM rds_warehouse_public.facts\nWHERE business_id='<id>' AND name IN ('sos_filings','sos_active','sos_match_boolean');\n\n-- See filing details:\nSELECT elem->>'state' AS state, elem->>'active' AS active,\n       elem->>'entity_type' AS entity_type, elem->>'filing_date' AS date\nFROM rds_warehouse_public.facts,\n     jsonb_array_elements(value->'value') AS elem\nWHERE business_id='<id>' AND name='sos_filings';"},
            {"label":"Filing Status","fact":"sos_filings[n].active","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT elem->>'active' AS active FROM rds_warehouse_public.facts,\n     jsonb_array_elements(value->'value') AS elem WHERE business_id='<id>' AND name='sos_filings';"},
            {"label":"Entity Jurisdiction Type","fact":"sos_filings[n].foreign_domestic","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT elem->>'foreign_domestic' AS jurisdiction FROM rds_warehouse_public.facts,\n     jsonb_array_elements(value->'value') AS elem WHERE business_id='<id>' AND name='sos_filings';"},
            {"label":"State","fact":"sos_filings[n].state","api":"/facts/business/:id/kyb","editable":False,
             ...
             "null_cause":"N/A if state absent from SoS record.","badges":[],"react_src":("ADMIN_PORTAL","microsites-main/packages/case/src/page/Cases/CaseDetails/Tabs/KYB/hooks/BusinessRegistrationTab/useBusinessRegistrationTabDetails.tsx",140,160,"State row"),"sql":"SELECT elem->>'state' FROM rds_warehouse_public.facts, jsonb_array_elements(value->'value') AS elem WHERE business_id='<id>' AND name='sos_filings';"},
        ...
        st.code("SELECT (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='shareholder_document';", language="sql")
    ...
    if ci_card == "🗺️ Addresses Card":
        ...
        ADDR_FIELDS = [
            ...
            {"label":"Reported Addresses + Verified / Deliverable badges","fact":"addresses + addresses_deliverable + address_verification","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT jsonb_array_length(value->'value') AS count, (value::json)->>'value' AS addrs\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='addresses';\n\nSELECT (value::json)->>'value' AS deliverable_addrs FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='addresses_deliverable';"},
        ]
        ...
        BN_FIELDS = [
            ...
            {"label":"Reported Names (from public records)","fact":"names_found","api":"/facts/business/:id/kyb","editable":False,
             ...
             "storage":["rds_warehouse_public.facts name='names_found' (array)"],"null_cause":"Empty if no vendor found any names.","badges":[],"react_src":("ADMIN_PORTAL","microsites-main/packages/case/src/page/Cases/CaseDetails/Tabs/KYB/components/BusinessNamesCard.tsx",1,30,"reported names section"),"sql":"SELECT (value::json)->>'value' AS names_found FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='names_found';"},
        ]
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    WEB_FIELDS = [
        {"label":"Website URL","fact":"website","api":"/facts/business/:id/details","editable":True,
         ...
         "sql":"SELECT name, (value::json)->>'value' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('website','website_found');"},
        ...
        {"label":"Status + badge","fact":"websiteData.status","api":"/verification/businesses/:id/website-data","editable":False,
         ...
         "sql":"-- Check Verdata raw response:\nSELECT response FROM integration_data.request_response WHERE business_id='<id>' AND platform_id=35 ORDER BY requested_at DESC LIMIT 1;"},
    ]
    ...
    if w_card == "📋 Watchlists Scanned Card":
        ...
        WLIST_FIELDS = [
            {"label":"Watchlist hit count badge + entity grouping","fact":"watchlist.value.metadata","api":"/facts/business/:id/kyb","editable":False,
             ...
             "sql":"SELECT name, jsonb_array_length(COALESCE(value->'value'->'metadata','[]'::jsonb)) AS hit_count\nFROM rds_warehouse_public.facts WHERE business_id='<id>' AND name IN ('watchlist','watchlist_hits','watchlist_raw');\n\n-- See individual hits:\nSELECT hit->>'type' AS hit_type, hit->'metadata'->>'title' AS list_name,\n       hit->>'entity_name' AS entity, hit->>'url' AS source_url\nFROM rds_warehouse_public.facts,\n     jsonb_array_elements(value->'value'->'metadata') AS hit\nWHERE business_id='<id>' AND name='watchlist_raw';"},
        ]
        ...
        st.code("""-- All watchlist hits for this business:
SELECT hit->>'type' AS hit_type,
       hit->'metadata'->>'title' AS list_name,
       hit->'metadata'->>'agency' AS agency,
       hit->>'entity_name' AS entity_matched,
       hit->>'url' AS source_link,
       hit->>'entity_type' AS entity_type,
       hit->>'list_country' AS country
FROM rds_warehouse_public.facts,
     jsonb_array_elements(value->'value'->'metadata') AS hit
WHERE business_id = '<business_id>'
  AND name = 'watchlist_raw'
ORDER BY hit->>'entity_name';""", language="sql")
...
SELECT
    f.name,
    f.(value::json)->>'value'                              AS winning_value,
    f.(value::json)->'source'->>'platformId'               AS winning_platform_id,
    f.(value::json)->'source'->>'confidence'               AS winning_confidence,
    f.(value::json)->'override'->>'value'                  AS analyst_override,
    f.(value::json)->'override'->>'userId'                 AS overridden_by,
    jsonb_array_length(COALESCE(f.(value::json)->'alternatives','[]'::jsonb)) AS num_alternatives,
    f.received_at
...
SQL_FACTS = (
    "SELECT name, (value::json)->>'value' AS value, "
    "(value::json)->'source'->>'platformId' AS winning_pid, "
    "(value::json)->'source'->>'confidence' AS confidence, "
    "(value::json)->'override'->>'value' AS analyst_override "
    "FROM rds_warehouse_public.facts "
...
3. ALWAYS provide SQL to verify data when relevant:
   - Facts: SELECT name, (value::json)->>'value', (value::json)->'source'->>'platformId' FROM rds_warehouse_public.facts WHERE business_id='<id>' AND name='<fact>';
   - SoS filings: SELECT elem->>'state', elem->>'active' FROM rds_warehouse_public.facts, jsonb_array_elements(value->'value') AS elem WHERE business_id='<id>' AND name='sos_filings';
   - Pipeline B: SELECT primary_naics_code, zi_c_naics6 FROM datascience.customer_files WHERE business_id='<id>';
4. Provide Python code when asked:
   import psycopg2, pandas as pd
   conn = psycopg2.connect(host='<host>',dbname='<db>',user='<u>',password='<pw>')
   df = pd.read_sql("SELECT name, value FROM rds_warehouse_public.facts WHERE business_id=%s AND name=%s", conn, params=['<id>','<fact>'])
